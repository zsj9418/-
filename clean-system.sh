...
get_dir_size() {
    ...
    [[ -d "$dir" ]] || { echo 0; return; }
    local size=0
    if size=$(du -sb "$dir" 2>/dev/null | head -1 | awk '{print $1}') && [[ "$size" =~ ^[0-9]+$ ]]; then
        echo "$size"
    elif size=$(du -sk "$dir" 2>/dev/null | head -1 | awk '{print $1}') && [[ "$size" =~ ^[0-9]+$ ]]; then
        echo $(( size * 1024 ))
    else
        echo 0
    fi
...
send_wecom_notify() {
    ...
    if [[ -n "$STATS_MODULES_LOG" ]]; then
        local module_lines
        module_lines=$(echo "$STATS_MODULES_LOG" | head -8)
        modules_section="
**清理明细:**
${module_lines}"
    fi
...
${log_hint}"
    local safe_content
    safe_content=$(printf '%s' "$content" | python3 -c 'import sys,json; print(json.dumps(sys.stdin.read())[1:-1])' 2>/dev/null)
    if [[ -z "$safe_content" ]]; then
        safe_content=$(printf '%s' "$content" | sed 's/\\/\\\\/g; s/"/\\"/g; s/$/\\n/' | tr -d '\n' | sed 's/\\n$//')
    fi
    local payload="{\"msgtype\":\"markdown\",\"markdown\":{\"content\":\"${safe_content}\"}}"
    local resp
...
setup_wecom() {
    ...
    while true; do
        read -rp "请粘贴 Webhook URL: " input_url
        ...
        debian)
            ...
            if [[ -n "$old_kernels" ]]; then
                ...
                if [[ $IS_CRON -eq 1 || $AGGRESSIVE -eq 1 ]]; then
                    ...
                    read -rp "  删除旧内核? [y/N]: " ans
                    [[ "${ans:-n}" =~ ^[Yy]$ ]] && do_clean=1
                fi
                if [[ $do_clean -eq 1 ]]; then
                    local kernel_list
                    kernel_list=$(echo "$old_kernels" | tr '\n' ' ')
                    safe_exec "DEBIAN_FRONTEND=noninteractive apt-get purge -y --auto-remove $kernel_list"
                    success "旧内核已清理"
                else
                    skip "跳过旧内核"
                fi
...
clean_package_manager() {
    ...
    case "$OS_TYPE" in
        debian)
            ...
            if [[ -n "$rc_pkgs" ]]; then
                local rc_list
                rc_list=$(echo "$rc_pkgs" | tr '\n' ' ')
                safe_exec "echo '$rc_list' | xargs dpkg --purge"
                success "残留配置: $(echo "$rc_pkgs" | wc -l | tr -d ' ') 个已清理"
            fi
        ...
        arch)
            ...
            if [[ -n "$orphans" ]]; then
                ...
                safe_exec "echo '$orphan_list' | pacman -Rns --noconfirm -"
                success "孤立包已清理"
            fi
            local after
        ...
        macos)
            if command -v brew &>/dev/null; then
                local brew_cache
                brew_cache=$(brew --cache 2>/dev/null || echo '/tmp/brew')
                local before
                before=$(get_dir_size "$brew_cache")
                safe_exec "brew cleanup --prune=all"
                safe_exec "brew autoremove"
                local after
                after=$(get_dir_size "$brew_cache")
                freed_est=$(( before - after ))
                (( freed_est < 0 )) && freed_est=0
...
clean_docker() {
    ...
    [[ -n "$docker_root" && -d "$docker_root" ]] && df_before=$(get_dir_size "$docker_root")
    ...
    if [[ $AGGRESSIVE -eq 1 || $IS_CRON -eq 1 ]]; then
        ...
        read -rp "  清理 Docker 未使用的卷? [y/N]: " ans
        [[ "${ans:-n}" =~ ^[Yy]$ ]] && safe_exec "docker volume prune -f"
        success "Docker 清理完成"
    fi
    local df_after=0
    [[ -n "$docker_root" && -d "$docker_root" ]] && df_after=$(get_dir_size "$docker_root")
...
clean_dev_caches() {
    ...
    if command -v yarn &>/dev/null; then
        found=1
        local yarn_dir yarn_sz=0
        yarn_dir=$(yarn cache dir 2>/dev/null || echo "$HOME/.yarn")
        [[ -d "$yarn_dir" ]] && yarn_sz=$(get_dir_size "$yarn_dir")
    ...
    if command -v bun &>/dev/null; then
        ...
        local bun_sz=0
        [[ -d "$bun_cache" ]] && bun_sz=$(get_dir_size "$bun_cache")
    ...
    if [[ $AGGRESSIVE -eq 1 ]]; then
        ...
        pycache_before=$(find /home /root /opt /srv -type d -name '__pycache__' 2>/dev/null | while read -r d; do get_dir_size "$d"; done | awk '{s+=$1}END{print s+0}')
        [[ "$pycache_before" =~ ^[0-9]+$ ]] || pycache_before=0
        safe_exec "find /home /root /opt /srv -type d -name '__pycache__' -exec rm -rf {} + 2>/dev/null"
    ...
    if command -v composer &>/dev/null; then
        ...
        success "Composer 已清理"
    fi
    if command -v gem &>/dev/null; then
        found=1
        safe_exec "gem cleanup"
        success "Ruby Gem 已清理"
    fi
...
clean_user_caches() {
    ...
    local total_freed=0
    local home_dirs=()
    get_home_dirs home_dirs
    if [[ ${#home_dirs[@]} -eq 0 ]]; then
        skip "未找到用户目录"
        append_module_stat "用户缓存" "0"
        return 0
    fi
    local hd
    for hd in "${home_dirs[@]}"; do
        find "$hd" -maxdepth 5 -type f \( -name '*.swp' -o -name '*.swo' -o -name '*~' -o -name '*.save' -o -name '*.bak' -o -name '#*#' \) -delete 2>/dev/null || true
    done
    success "编辑器临时文件已清理"
    for hd in "${home_dirs[@]}"; do
        find "$hd" -type d -name 'thumbnails' -exec rm -rf {} + 2>/dev/null || true
    done
    if [[ $IS_MACOS -eq 0 ]]; then
        for hd in "${home_dirs[@]}"; do
            find "$hd" -path '*/.local/share/Trash/*' -delete 2>/dev/null || true
            local trash_dir="$hd/.local/share/Trash"
            [[ -d "$trash_dir" ]] && rm -rf "${trash_dir:?}"/* 2>/dev/null || true
        done
        success "回收站已清空"
    fi
    if [[ $IS_MACOS -eq 1 ]]; then
        for hd in "${home_dirs[@]}"; do
            ...
            rm -rf "${hd:?}/.Trash/"* 2>/dev/null || true
    ...
    for hd in "${home_dirs[@]}"; do
        ...
        find "$hd/.cache" -type f -atime +7 -delete 2>/dev/null || true
        find "$hd/.cache" -type d -empty -delete 2>/dev/null || true
    ...
    if [[ $IS_MACOS -eq 1 ]]; then
        for hd in "${home_dirs[@]}"; do
            ...
            local mac_cache_before mac_cache_after
            mac_cache_before=$(get_dir_size "$hd/Library/Caches")
            find "$hd/Library/Caches" -type f -atime +7 -delete 2>/dev/null || true
            mac_cache_after=$(get_dir_size "$hd/Library/Caches")
            local mac_diff=$(( mac_cache_before - mac_cache_after ))
            (( mac_diff > 0 )) && total_freed=$(( total_freed + mac_diff ))
        done
    ...
    if [[ $AGGRESSIVE -eq 1 ]]; then
        for hd in "${home_dirs[@]}"; do
            local hist_file
            for hist_file in "$hd/.bash_history" "$hd/.zsh_history" "$hd/.ash_history"; do
                [[ -f "$hist_file" ]] && command -v truncate &>/dev/null && truncate -s 0 "$hist_file" 2>/dev/null || true
            done
        done
        success "Shell 历史已清空"
    fi
...
disk_usage_analysis() {
    ...
    find / -xdev -type f -size +100M 2>/dev/null | head -30 | while read -r f; do
        du -h "$f" 2>/dev/null
    done | sort -rh | head -10 | awk '{printf "  %-8s %s\n", $1, $2}'
    echo ""
}
clean_system() {
    detect_system
...
setup_cron() {
    ...
    case "${freq:-2}" in
        ...
        3) schedule="0 3 * * 0" ;;
        4) schedule="0 3 1 * *" ;;
        5) read -rp "输入 cron 表达式 (分 时 日 月 周): " schedule ;;
        *) schedule="0 3 */2 * *" ;;
    esac
    ...
    local cron_file="/etc/cron.d/clean_system"
    if [[ -d /etc/cron.d ]] && [[ -w /etc/cron.d ]]; then
        ...
        local tmpfile
        tmpfile=$(mktemp)
        (crontab -l 2>/dev/null || true) | grep -v "$script_path" > "$tmpfile" || true
...
uninstall() {
    local script_path
    script_path="$(realpath "$0" 2>/dev/null || echo "$0")"
    [[ -f /etc/cron.d/clean_system ]] && {
        rm -f /etc/cron.d/clean_system
        success "/etc/cron.d/clean_system 已移除"
    }
...
HELPEOF
    ...
    if [[ $EUID -ne 0 ]]; then
        [[ $IS_CRON -eq 0 ]] && yellow "提示: 建议使用 sudo 运行以获得完整清理效果"
    fi
    init_log
