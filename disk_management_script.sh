...
install_dependencies() {
    echo -e "${YELLOW}正在检查并安装缺失的依赖...${NC}"
    if command -v apt-get &> /dev/null; then
        sudo apt-get update
        sudo apt-get install -y dosfstools e2fsprogs ntfs-3g >> "$LOG_FILE" 2>&1
    elif command -v yum &> /dev/null; then
        sudo yum install -y dosfstools e2fsprogs ntfs-3g >> "$LOG_FILE" 2>&1
    elif command -v brew &> /dev/null; then
        brew install e2fsprogs >> "$LOG_FILE" 2>&1
    else
