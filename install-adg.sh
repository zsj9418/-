...
#默认容器路径
default_path () {
    # 提示用户输入容器路径，如果直接回车，则使用默认路径
    echo -e '\e[92m请输入容器存储路径，直接回车使用默认路径 \e[0m /opt/docker/'
    read -p "路径: " input_path
    save_path=${input_path:-/opt/docker/} # 如果用户未输入，使用默认路径

    # 检查路径是否为绝对路径
    if [[ "$save_path" != /* ]]; then
        echo -e '\e[91m路径无效！路径必须是以 "/" 开头的绝对路径。\e[0m'
        default_path # 重新调用函数，直到用户输入正确路径或接受默认路径
    fi

    # 如果路径不存在，则自动创建
    if [[ ! -d "$save_path" ]]; then
        mkdir -p "$save_path"
        echo -e '\e[92m路径已创建: \e[0m' "$save_path"
    else
        echo -e '\e[92m使用的路径为: \e[0m' "$save_path"
    fi
}
...
#固化容器路径
curing_path () {
    sed -i "4s|save_path=.*|save_path=$swap_path|" adg.sh
