...
function _http_get() {
  ...
  if command -v curl &>/dev/null; then
    curl -sSL --connect-timeout "$timeout" --max-time "$timeout" "$url" 2>/dev/null
...
function _http_code() {
  local url="$1" timeout="${2:-$NET_TIMEOUT}"
  if command -v curl &>/dev/null; then
    curl -sSL --connect-timeout "$timeout" --max-time "$timeout" -o /dev/null -w "%{http_code}" "$url" 2>/dev/null || echo "000"
  else
    wget -qO /dev/null --timeout="$timeout" --server-response "$url" 2>&1 | awk '/^  HTTP/{print $2}' | tail -1 || echo "000"
  fi
}
function _net_reachable() {
  local host="$1" timeout="${2:-$NET_TIMEOUT}"
  local code; code=$(_http_code "https://${host}/v2/" "$timeout")
  [[ "$code" =~ ^(200|401|403|301|302)$ ]] && return 0
  command -v ping &>/dev/null && ping -c 1 -W "$timeout" "$host" &>/dev/null && return 0
  return 1
}
function _net_ms() {
  local url="$1" timeout="${2:-$NET_TIMEOUT}"
  local start; start=$(date +%s%3N 2>/dev/null || date +%s)
  _http_code "$url/v2/" "$timeout" >/dev/null 2>&1
  local end; end=$(date +%s%3N 2>/dev/null || date +%s)
  echo $(( end - start ))
}
function _hub_ms() {
  local url="$1" timeout="${2:-5}"
  local start; start=$(date +%s%3N 2>/dev/null || date +%s)
  local code; code=$(_http_code "$url" "$timeout")
  local end; end=$(date +%s%3N 2>/dev/null || date +%s)
  local ms=$(( end - start ))
  [[ "$code" =~ ^(200|301|302|401|403)$ ]] && echo "$ms" || echo "9999"
}
function _best_dockerhub_mirror() {
  local best_url="" best_ms=99999
  for mirror in "${DOCKERHUB_MIRRORS[@]}"; do
    local ms; ms=$(_hub_ms "$mirror" 3)
    if [[ "$ms" -lt "$best_ms" ]]; then
      best_ms="$ms"; best_url="$mirror"
    fi
  done
  echo "$best_url"
}
function _best_ghcr_mirror() {
  for mirror in "${GHCR_MIRRORS[@]}"; do
    _net_reachable "$mirror" 3 && { echo "$mirror"; return 0; }
  done
  return 1
}
function _replace_ghcr_host() {
  local image="$1" new_host="$2"
  echo "${image/ghcr.io/$new_host}"
}
function _is_ghcr_image() {
  [[ "${1%%/*}" == "ghcr.io" ]]
}
function _is_dockerhub_image() {
  local image="$1"
  local registry="${image%%/*}"
  [[ "$registry" == "docker.io" ]] && return 0
  [[ "$image" != *"."*"/"* ]] && return 0
  return 1
}
function _net_detect() {
  yellow "检测网络连通性..."
  local ghcr_ok=false hub_ok=false
  _net_reachable "ghcr.io"              3 && ghcr_ok=true || true
  _net_reachable "registry-1.docker.io" 3 && hub_ok=true  || true
  $ghcr_ok  && green "  ghcr.io       : 可达" || yellow "  ghcr.io       : 不可达（将自动使用镜像）"
  $hub_ok   && green "  Docker Hub    : 可达" || yellow "  Docker Hub    : 不可达（将自动使用镜像）"
  echo ""
}
function _smart_pull() {
  ...
  if _is_ghcr_image "$image"; then
    if _net_reachable "ghcr.io" 3; then
      yellow "  ghcr.io 可达，直接拉取..."
      docker pull $pa "$image" 2>/dev/null && { green "  ✅ 拉取成功: $image"; return 0; }
      yellow "  直接拉取失败，切换镜像重试..."
    else
      yellow "  ghcr.io 不可达，自动切换镜像..."
    fi
    local mhost
    if mhost=$(_best_ghcr_mirror 2>/dev/null); then
      local mimg; mimg=$(_replace_ghcr_host "$image" "$mhost")
      yellow "  使用镜像: $mimg"
      if docker pull $pa "$mimg" 2>/dev/null; then
        docker tag "$mimg" "$image" 2>/dev/null || true
        green "  ✅ 拉取成功（via $mhost）"
        return 0
      fi
      yellow "  镜像 $mhost 失败，尝试其他镜像..."
    fi
    for mhost in "${GHCR_MIRRORS[@]}"; do
      local mimg; mimg=$(_replace_ghcr_host "$image" "$mhost")
      yellow "  尝试: $mimg"
      if docker pull $pa "$mimg" 2>/dev/null; then
        docker tag "$mimg" "$image" 2>/dev/null || true
    ...
    red "  ❌ ghcr.io 所有镜像均失败，请配置加速器或检查网络。"
    return 1
  fi
  yellow "  直接拉取..."
  if docker pull $pa "$image" 2>/dev/null; then
    green "  ✅ 拉取成功: $image"
    return 0
  fi
  yellow "  直接拉取失败，自动测速选最优镜像（约10秒）..."
  local configured_mirrors=()
  while IFS= read -r m; do [[ -n "$m" ]] && configured_mirrors+=("$m"); done < <(_read_current_mirrors)
  if [[ ${#configured_mirrors[@]} -gt 0 ]]; then
    yellow "  使用已配置加速器重试..."
    docker pull $pa "$image" 2>/dev/null && { green "  ✅ 拉取成功（via 已配置镜像）"; return 0; }
  fi
  local best_mirror; best_mirror=$(_best_dockerhub_mirror)
  if [[ -n "$best_mirror" ]]; then
    yellow "  最优镜像: $best_mirror，写入配置..."
    _write_mirrors_to_daemon "[\"$best_mirror\"]" 2>/dev/null || true
    _restart_docker_daemon 2>/dev/null || true
    sleep 2
    docker pull $pa "$image" 2>/dev/null && { green "  ✅ 拉取成功（via $best_mirror）"; return 0; }
  fi
  red "  ❌ 所有拉取方式均失败，请手动配置加速器（主菜单选项8）。"
  return 1
}
function pull_image_with_retry() {
  local img="$1"
  if ! _smart_pull "$img"; then
    read -rp "拉取失败，重试？(y/n，默认 n): " rp </dev/tty
    if [[ "${rp:-n}" =~ ^[Yy]$ ]]; then
      _smart_pull "$img" || { red "仍然失败，请检查网络或手动配置加速器。"; return 1; }
    else
  ...
  if [[ -f /etc/os-release ]]; then
    ...
    case "$id" in
      ...
      centos|rhel|rocky|almalinux)
        OS="$id"; [[ -f /usr/bin/dnf ]] && PACKAGE_MANAGER="dnf" || PACKAGE_MANAGER="yum" ;;
...
function install_dependency() {
  ...
  else red "$pkg 安装失败，请手动安装。"; return 1
  fi
}
function check_base_dependencies() {
  ...
  if ! command -v ss &>/dev/null && ! command -v netstat &>/dev/null; then
    ...
    install_dependency "netstat" "$netstat_pkg" || \
      install_dependency "ss" "$ss_pkg" || yellow "端口检查工具安装失败，跳过。"
  fi
}
function check_docker_dependencies() {
  ...
  if [[ "$needs_repo" == true && "$OS" != "openwrt" ]]; then
    ...
    if [[ -n "$repo_pkg" ]]; then
      local ic="$PACKAGE_MANAGER install -y $repo_pkg"
      [[ "$EUID" -ne 0 ]] && ic="sudo $ic"
      eval "$ic" || true
      local fc="$repo_cmd"
      [[ "$EUID" -ne 0 && "$fc" != sudo* ]] && fc="sudo $fc"
      eval "$fc" || yellow "添加 Docker 仓库失败，继续..."
    fi
  fi
  ...
  if [[ "$OS" == "openwrt" ]]; then
    /etc/init.d/dockerd status 2>/dev/null | grep -q "running" || {
      ...
      /etc/init.d/dockerd start  || yellow "请手动启动：/etc/init.d/dockerd start"
    }
  elif command -v systemctl &>/dev/null; then
    systemctl is-active --quiet docker || {
      local sc="systemctl enable --now docker"
      [[ "$EUID" -ne 0 ]] && sc="sudo $sc"
      eval "$sc" || yellow "请手动启动 Docker 服务。"
    }
  fi
  ...
  if docker compose version &>/dev/null; then
    ...
    green "Docker Compose：$(docker-compose --version)"
  else
    yellow "安装 Docker Compose 独立版..."
    local cv
    cv=$(_http_get "https://api.github.com/repos/docker/compose/releases/latest" 5 \
    ...
    command -v curl &>/dev/null \
      && dl="curl -sSL \"$url\" -o /tmp/docker-compose" \
      || dl="wget -q \"$url\" -O /tmp/docker-compose"
    ...
    eval "$mv_cmd" && eval "$cx_cmd" \
      && green "docker-compose $cv 安装成功。" \
      || { red "安装失败。"; rm -f /tmp/docker-compose; return 1; }
  fi
}
function check_user_permission() {
  ...
  if groups "$USER" | grep -q '\bdocker\b'; then
    green "用户 $USER 已在 docker 组。"
  else
    yellow "将用户 $USER 加入 docker 组..."
    if command -v sudo &>/dev/null; then
      sudo usermod -aG docker "$USER" \
        && green "已加入，请执行 'newgrp docker' 或重新登录生效。" \
        || red "加入失败，请手动：sudo usermod -aG docker $USER"
    else
      red "缺少 sudo，请手动：usermod -aG docker $USER"
    fi
  fi
}
function find_available_port() {
  ...
  while $ck | grep -Eq "[:.\[]${p}[[:space:]]+"; do
    p=$((p + 1)); [[ "$p" -gt 65535 ]] && { p=${1:-3000}; break; }
  done
  echo "$p"
}
function validate_port() {
  local hint=${1:-3000}
  local sug; sug=$(find_available_port "$hint")
  green "建议端口：$sug"
  local attempts=0
  while true; do
    attempts=$((attempts + 1))
    [[ "$attempts" -gt 10 ]] && { red "端口错误次数过多，退出。"; return 1; }
    read -rp "输入端口（留空使用 $sug）：" up </dev/tty
    PORT=${up:-$sug}
...
function _do_pack() {
  ...
  case "$pack_type" in
    ...
    zip)
      local prev="$PWD"; cd "$src_dir" || { rm -f "$err_file"; return 1; }
      zip -qr "$abs_out" "$src_base" 2>"$err_file"; local zrc=$?
      cd "$prev" || true
      if [[ $zrc -ne 0 ]]; then
        [[ -s "$err_file" ]] && while IFS= read -r l; do red "  $l"; done < "$err_file"
...
function _select_backup_root() {
  ...
  case "${bc:-1}" in
    2) backup_root="/tmp" ;;
    3) read -rp "目录路径：" custom_dir </dev/tty; backup_root="${custom_dir:-$HOME}" ;;
    *) backup_root="$HOME" ;;
  esac
...
function _scan_backup_files() {
  ...
  for d in "${scan_dirs[@]}"; do
    [[ -d "$d" ]] || continue
    find "$d" -maxdepth 3 \
      \( -name "${prefix}-*.tar.gz" -o -name "${prefix}-*.tar.bz2" \
      -o -name "${prefix}-*.tar.xz" -o -name "${prefix}-*.tar" \
      -o -name "${prefix}-*.zip" \) 2>/dev/null | sort -r
  done
}
...
PYEOF
  ...
  if [[ -n "$new_content" ]]; then
    echo "$new_content" | $write_cmd "$daemon_json" >/dev/null; return $?
  fi
...
function _restart_docker_daemon() {
  ...
  if command -v systemctl &>/dev/null; then
    ...
    yellow "请手动重启 Docker 服务。"
  fi
}
function _copy_volume_to_dir() {
  local volume_name="$1" dest_dir="$2"
  mkdir -p "$dest_dir"
  yellow "  策略1：直接读取卷路径..."
  ...
  if [[ -n "$vol_path" && -d "$vol_path" ]]; then
    local ok=false
    if [[ "$EUID" -eq 0 ]]; then
      cp -a "$vol_path/." "$dest_dir/" 2>/dev/null && ok=true
    elif command -v sudo &>/dev/null; then
      sudo cp -a "$vol_path/." "$dest_dir/" 2>/dev/null && \
        sudo chown -R "$(id -u):$(id -g)" "$dest_dir" 2>/dev/null && ok=true || true
    fi
    if $ok; then
    ...
    yellow "  策略1 失败（路径不可访问），下一策略..."
  fi
  yellow "  策略2：使用本地轻量镜像..."
  local helper=""
  for c in "alpine:latest" "alpine" "busybox:latest" "busybox"; do
    docker image inspect "$c" &>/dev/null && { helper="$c"; break; }
  done
  if [[ -n "$helper" ]]; then
    if docker run --rm -v "${volume_name}:/source:ro" -v "${dest_dir}:/backup" \
        "$helper" sh -c "cp -a /source/. /backup/" 2>/dev/null; then
      local fc; fc=$(find "$dest_dir" -type f 2>/dev/null | wc -l || echo 0)
  ...
  yellow "  策略3：tar 流式导出..."
  local tar_out="${dest_dir}/_vol_$$.tar" tok=false
  docker run --rm -v "${volume_name}:/source:ro" busybox \
    sh -c "tar cf - -C /source ." > "$tar_out" 2>/dev/null && tok=true
  if ! $tok; then
    docker run --rm -v "${volume_name}:/source:ro" alpine \
      sh -c "tar cf - -C /source ." > "$tar_out" 2>/dev/null && tok=true
  fi
  if $tok; then
  ...
  yellow "  策略4：拉取 alpine..."
  _smart_pull "alpine:latest" &>/dev/null && \
    docker run --rm -v "${volume_name}:/source:ro" -v "${dest_dir}:/backup" \
      alpine sh -c "cp -a /source/. /backup/" 2>/dev/null && \
    { green "  策略4 成功"; return 0; }
  red "  ❌ 所有备份策略失败，请检查 Docker 卷或网络。"
  return 1
}
function _copy_dir_to_volume() {
  ...
  if [[ -n "$vol_path" && -d "$vol_path" ]]; then
    ...
    if ! $ok && command -v sudo &>/dev/null; then
      sudo cp -a "$src_dir/." "$vol_path/" 2>/dev/null && ok=true
    fi
  ...
  if [[ -n "$helper" ]]; then
    docker run --rm -v "${volume_name}:/target" -v "${src_dir}:/source:ro" \
      "$helper" sh -c "rm -rf /target/* /target/.[!.]* 2>/dev/null; cp -a /source/. /target/" 2>/dev/null \
...
function _fetch_versions_ghcr() {
  ...
  local token; token=$(_http_get "https://ghcr.io/token?scope=repository:${image_path}:pull&service=ghcr.io" 5 \
    | grep -o '"token":"[^"]*"' | cut -d'"' -f4 2>/dev/null || true)
  [[ -z "$token" ]] && return 1
  local resp=""
  command -v curl &>/dev/null && \
    resp=$(curl -sSL --connect-timeout 8 --max-time 8 \
...
function _fetch_versions_dockerhub() {
  local repo="$1" limit="${2:-8}"
  local resp; resp=$(_http_get \
    "https://hub.docker.com/v2/repositories/${repo}/tags?page_size=50&ordering=last_updated" 8) || return 1
  [[ -z "$resp" ]] && return 1
...
function select_image_version() {
  ...
  local clean=()
  for v in "${versions[@]}"; do [[ -n "$v" ]] && clean+=("$v"); done
  ...
  if [[ -z "$vc" || "$vc" == "1" ]]; then
    SELECTED_IMAGE="${img_base}:latest"
  elif [[ "$vc" =~ ^[0-9]+$ ]] && [[ "$vc" -ge 2 && "$vc" -lt "$midx" ]]; then
    SELECTED_IMAGE="${img_base}:${versions[$((vc-2))]}"
  elif [[ "$vc" == "$midx" ]]; then
    read -rp "输入版本号（如 v1.2.3）：" mv </dev/tty
...
function _prompt_backup_before_action() {
  ...
  read -rp "继续${action}？(y/n，默认 y)：" pc </dev/tty
  [[ ! "${pc:-y}" =~ ^[Yy]$ ]] && { yellow "已取消。"; return 1; }
  return 0
}
function _backup_bind_service() {
  ...
  if docker ps --format '{{.Names}}' | grep -q "^${container_name}$"; then
    was_running=true; docker stop "$container_name" &>/dev/null || true
    green "  容器已暂停"
  else
    yellow "  容器未运行，直接备份"
  fi
  yellow "2/3 复制数据..."
  mkdir -p "$btmp/data"
  if cp -a "$data_dir/." "$btmp/data/" 2>/dev/null; then
    local copied; copied=$(find "$btmp/data" -type f 2>/dev/null | wc -l || echo 0)
    green "  复制完成（$copied 个文件）"
  else
    red "  复制失败"
...
EOF
  [[ "$was_running" == true ]] && {
    yellow "  重启容器..."
    docker start "$container_name" &>/dev/null && green "  容器已恢复" || yellow "  请手动：docker start $container_name"
  }
  yellow "3/3 打包压缩（$pack_type）..."
  if _do_pack "$backup_root" "$bname" "$barch" "$pack_type"; then
    rm -rf "$btmp"
    local size; size=$(du -sh "$barch" 2>/dev/null | cut -f1 || echo "未知")
    echo ""
...
function _restore_bind_service() {
  ...
  case "${sc:-2}" in
    ...
    *)
      ...
      if [[ ${#found_files[@]} -eq 0 ]]; then
        ...
        if [[ -z "$fc" ]]; then
          read -rp "手动输入路径：" barch </dev/tty
        elif [[ "$fc" =~ ^[0-9]+$ && "$fc" -ge 1 && "$fc" -le ${#found_files[@]} ]]; then
          barch="${found_files[$((fc-1))]}"
        else
          red "无效选项。"; return 1
        fi
      fi
  ...
  [[ -z "$barch" || ! -f "$barch" ]] && { red "备份文件不存在：$barch"; return 1; }
  green "使用备份：$barch"
  local rtmp="/tmp/${backup_prefix}_restore_$$"
  mkdir -p "$rtmp"
  yellow "1/4 解压..."
  _do_unpack "$barch" "$rtmp" || { red "解压失败。"; rm -rf "$rtmp"; return 1; }
  ...
  local rbase; rbase=$(dirname "$rinfo")
  [[ -f "$rbase/backup_info.txt" ]] && { echo ""; cyan "备份信息："; cat "$rbase/backup_info.txt"; echo ""; }
  [[ ! -d "$rbase/data" ]] && { red "备份中无 data 目录！"; rm -rf "$rtmp"; return 1; }
  yellow "2/4 停止容器..."
  docker ps --format '{{.Names}}' | grep -q "^${container_name}$" && \
    { docker stop "$container_name" &>/dev/null; green "  容器已停止"; } || yellow "  容器未运行"
  yellow "3/4 恢复数据..."
  if [[ -d "$data_dir" ]]; then
    local bak_old="${data_dir}_old_$(date +%Y%m%d_%H%M%S)"
    mv "$data_dir" "$bak_old" 2>/dev/null || { red "无法移动旧数据目录"; rm -rf "$rtmp"; return 1; }
    green "  旧数据已保留：$bak_old"
  fi
  mkdir -p "$data_dir"
  if cp -a "$rbase/data/." "$data_dir/" 2>/dev/null; then
    local fc; fc=$(find "$data_dir" -type f 2>/dev/null | wc -l || echo 0)
    green "  恢复完成（$fc 个文件）"
  else
    red "  数据恢复失败"; rm -rf "$rtmp"; return 1
  fi
  rm -rf "$rtmp"
  yellow "4/4 启动容器..."
  docker ps -a --format '{{.Names}}' | grep -q "^${container_name}$" && \
    { docker start "$container_name" &>/dev/null && green "  容器已启动" || red "  请手动：docker start $container_name"; } || \
    yellow "  容器不存在，请重新部署（数据已恢复）。"
  echo ""
  green "✅ $svc_label 恢复完成"
}
function _list_bind_service_backups() {
  local svc_label="$1" backup_prefix="$2"
  echo ""
  cyan "$svc_label 备份列表："
  local found=false
...
function _backup_volume_service() {
  ...
  echo ""
  cyan "备份：$svc_label（Docker 命名卷：$volume_name）"
  if ! docker volume inspect "$volume_name" &>/dev/null; then
    red "数据卷 $volume_name 不存在，请确认服务已正确部署。"; return 1
  fi
  local pack_type; pack_type=$(_check_pack_tools)
  [[ "$pack_type" == "none" ]] && { red "未找到打包工具！"; return 1; }
  local backup_root; backup_root=$(_select_backup_root) || return 1
  _check_disk_space "$backup_root" 102400 || return 1
  local stamp; stamp=$(date +%Y%m%d_%H%M%S)
  local bname="${backup_prefix}-${stamp}"
  local btmp="${backup_root}/${bname}"
  local barch="${backup_root}/${bname}.${pack_type}"
  mkdir -p "$btmp/data"
  yellow "1/3 暂停容器保证数据一致性..."
  local was_running=false
  if docker ps --format '{{.Names}}' | grep -q "^${container_name}$"; then
    was_running=true
  ...
  yellow "2/3 导出数据卷..."
  if ! _copy_volume_to_dir "$volume_name" "$btmp/data"; then
    red "  数据卷导出失败"
    [[ "$was_running" == true ]] && docker start "$container_name" &>/dev/null || true
    rm -rf "$btmp"; return 1
  fi
...
function _restore_volume_service() {
  local svc_label="$1" container_name="$2" volume_name="$3" backup_prefix="$4"
  echo ""
  cyan "恢复：$svc_label（Docker 命名卷：$volume_name）"
  ...
  if docker volume inspect "$volume_name" &>/dev/null; then
    ...
    docker volume create "$volume_name" &>/dev/null && green "  数据卷已创建"
  fi
  yellow "3/4 恢复数据到卷..."
  if ! _copy_dir_to_volume "$rbase/data" "$volume_name"; then
    red "  数据卷恢复失败"; rm -rf "$rtmp"; return 1
  fi
  green "  数据卷恢复完成"
  rm -rf "$rtmp"
  yellow "4/4 启动容器..."
  docker ps -a --format '{{.Names}}' | grep -q "^${container_name}$" && \
    { docker start "$container_name" &>/dev/null && green "  容器已启动" || red "  请手动：docker start $container_name"; } || \
    yellow "  容器不存在，请通过主菜单重新部署（数据卷已恢复）。"
  echo ""
  green "✅ $svc_label 恢复完成"
}
...
function _mirror_auto_setup() {
  ...
  for mirror in "${DOCKERHUB_MIRRORS[@]}"; do
    printf "  %-42s" "$mirror"
    local ms; ms=$(_hub_ms "$mirror" 5)
    if [[ "$ms" -lt 9000 ]]; then
      printf "\e[32m%dms\e[0m\n" "$ms"
      results+=("$ms|$mirror")
      [[ "$ms" -lt "$best_time" ]] && { best_time="$ms"; best_mirror="$mirror"; }
    else
      printf "\e[31m超时\e[0m\n"
    fi
  done
  echo ""
  [[ -z "$best_mirror" ]] && { red "所有加速器不可达。"; return 1; }
  green "最优：$best_mirror（${best_time}ms）"
  local selected=()
  for entry in "${results[@]}"; do
    local ms="${entry%%|*}" url="${entry##*|}"
    [[ "$ms" -lt 5000 ]] && selected+=("$url")
  done
  [[ ${#selected[@]} -eq 0 ]] && selected=("$best_mirror")
  local mj="[" first=true
  for m in "${selected[@]}"; do
    $first && mj+="\"$m\"" || mj+=", \"$m\""; first=false
  done
  mj+="]"
  read -rp "写入配置并重启 Docker？(y/n，默认 y)：" cf </dev/tty
  [[ ! "${cf:-y}" =~ ^[Yy]$ ]] && { yellow "已取消。"; return 0; }
  _write_mirrors_to_daemon "$mj" || { red "写入失败。"; return 1; }
  green "✅ 加速器已写入"
...
function _mirror_test_all() {
  ...
  for mirror in "${DOCKERHUB_MIRRORS[@]}"; do
    printf "  %-42s" "$mirror"
    local ms; ms=$(_hub_ms "$mirror" 5)
    [[ "$ms" -lt 9000 ]] && printf "\e[32m%dms\e[0m\n" "$ms" || printf "\e[31m超时\e[0m\n"
  done
}
function _mirror_add_custom() {
  echo ""
  read -rp "输入加速器地址（多个空格分隔，留空取消）：" custom_input </dev/tty
  ...
  local all=("${existing[@]+"${existing[@]}"}" "${new_mirrors[@]}")
  local seen=() unique=()
  for m in "${all[@]}"; do
    local dup=false
    [[ ${#seen[@]} -gt 0 ]] && for s in "${seen[@]}"; do [[ "$s" == "$m" ]] && dup=true && break; done
    $dup || { unique+=("$m"); seen+=("$m"); }
  done
  local mj="[" first=true
  for m in "${unique[@]}"; do $first && mj+="\"$m\"" || mj+=", \"$m\""; first=false; done
  mj+="]"
  _write_mirrors_to_daemon "$mj" || { red "写入失败。"; return 1; }
  green "✅ 加速器已添加"
  _restart_docker_daemon || return 1
...
function _mirror_clear_all() {
  red "⚠️  将清除所有加速器配置！"
  read -rp "确认？(y/n，默认 n)：" cf </dev/tty
  [[ ! "${cf:-n}" =~ ^[Yy]$ ]] && { yellow "已取消。"; return 0; }
  _write_mirrors_to_daemon "[]" || { red "写入失败。"; return 1; }
  green "✅ 加速器已清除"
  _restart_docker_daemon || return 1
}
function _mirror_show_daemon_json() {
  local dj; dj=$(_get_daemon_json_path)
  echo ""
  cyan "/etc/docker/daemon.json："
  [[ -f "$dj" ]] && cat "$dj" || yellow "文件不存在"
}
function general_backup_menu() {
  while true; do
    echo ""
    cyan "One-API / New-API / New-API Horizon 备份/恢复"
    echo "──────────────────────────────────────────────"
    ...
    [[ -d "$HOME/one-api-data" ]]         && s1+=" [数据存在]"
    [[ -d "$HOME/one-api-mysql-logs" ]]   && s2+=" [数据存在]"
    [[ -d "$HOME/new-api-data" ]]         && s3+=" [数据存在]"
    [[ -d "$HOME/new-api-horizon-data" ]] && s4+=" [数据存在]"
    echo "  One-API SQLite   [$s1]"
    echo "  1. 备份  2. 恢复  3. 列表"
    echo ""
    echo "  One-API MySQL    [$s2]"
    echo "  4. 备份  5. 恢复  6. 列表"
    echo "  ℹ️  MySQL数据请用：mysqldump -u用户 -p 库名 > backup.sql"
    echo ""
    echo "  New-API          [$s3]"
    echo "  7. 备份  8. 恢复  9. 列表"
    echo ""
    echo "  New-API Horizon  [$s4]"
    echo "  10. 备份  11. 恢复  12. 列表"
    echo ""
    echo "  0. 返回"
    echo ""
...
function omniroute_backup_menu() {
  while true; do
    echo ""
    cyan "OmniRoute 备份/恢复"
    yellow "⚠️  OmniRoute 使用 Docker 命名卷存储，备份时需要导出卷数据"
    echo "──────────────────────────────────────────────"
    local s="未部署"
    docker ps -a --format '{{.Names}}' | grep -q "^omniroute$" && s="已部署"
    docker volume inspect omniroute-data &>/dev/null             && s+=" [卷存在]"
    echo "  OmniRoute [$s]"
    echo ""
    echo "  1. 备份（数据卷导出打包）"
    echo "  2. 恢复（覆盖数据卷并启动）"
...
function freellmapi_backup() {
  ...
  cp "$key_file" "$btmp/.freellmapi_encryption_key" || { red "复制失败"; rm -rf "$btmp"; return 1; }
  chmod 600 "$btmp/.freellmapi_encryption_key"
  green "  KEY 已备份"
  yellow "2/4 备份配置文件..."
  ...
  if docker volume inspect freellmapi-data &>/dev/null; then
    ...
    $was_running && { yellow "  暂停容器..."; docker stop freellmapi &>/dev/null || true; }
    mkdir -p "$btmp/data"
    if ! _copy_volume_to_dir "freellmapi-data" "$btmp/data"; then
      red "  数据卷备份失败"
      $was_running && docker start freellmapi &>/dev/null || true
      rm -rf "$btmp"; return 1
    fi
    if $was_running; then
    ...
    yellow "  未找到 freellmapi-data 卷，跳过"
  fi
  yellow "4/4 打包压缩..."
...
EOF
  if _do_pack "$backup_root" "$bname" "$barch" "$pack_type"; then
    ...
    green "   文件：$barch  大小：$size"
    yellow "   ⚠️  请将备份文件保存到安全位置！"
  else
    red "打包失败！临时目录：$btmp"; return 1
  fi
}
function freellmapi_restore() {
  ...
  if docker ps -a --format '{{.Names}}' | grep -q "^freellmapi$"; then
    ...
    [[ ! "${cc:-n}" =~ ^[Yy]$ ]] && { yellow "已取消。"; return 0; }
    [[ -f "$compose_dir/docker-compose.yml" ]] && {
      cd "$compose_dir" || true; docker compose down 2>/dev/null || true; cd - >/dev/null || true
    }
  ...
  case "${sc:-2}" in
    ...
    *)
      ...
      if [[ ${#found_files[@]} -eq 0 ]]; then
        yellow "未找到。"; read -rp "手动输入：" barch </dev/tty
      else
        local i=1
        for f in "${found_files[@]}"; do
          local sz; sz=$(du -sh "$f" 2>/dev/null | cut -f1 || echo "?")
        ...
        read -rp "选择编号（留空手动输入）：" fc </dev/tty
        [[ -z "$fc" ]] && read -rp "手动输入：" barch </dev/tty || {
          [[ "$fc" =~ ^[0-9]+$ && "$fc" -ge 1 && "$fc" -le ${#found_files[@]} ]] \
            && barch="${found_files[$((fc-1))]}" || { red "无效选项。"; return 1; }
        }
      fi
      ;;
  esac
  [[ -z "$barch" || ! -f "$barch" ]] && { red "备份文件不存在：$barch"; return 1; }
  green "使用：$barch"
  local rtmp="/tmp/freellmapi_restore_$$"
  mkdir -p "$rtmp"
  yellow "1/5 解压..."
  _do_unpack "$barch" "$rtmp" || { red "解压失败。"; rm -rf "$rtmp"; return 1; }
  ...
  local rbase; rbase=$(dirname "$rinner")
  green "  解压成功"
  local bak_image=""
  if [[ -f "$rbase/backup_info.txt" ]]; then
    cyan "备份信息："; cat "$rbase/backup_info.txt"; echo ""
  ...
  if [[ -f "$rbase/docker-compose.yml" ]]; then
    cp "$rbase/docker-compose.yml" "$compose_dir/docker-compose.yml"
    [[ -n "$bak_image" ]] && sed -i "s|image:.*freellmapi.*|image: ${bak_image}|" "$compose_dir/docker-compose.yml" || true
    green "  docker-compose.yml 已恢复"
  fi
  yellow "4/5 恢复数据卷..."
  if [[ -d "$rbase/data" ]]; then
    docker volume inspect freellmapi-data &>/dev/null \
      && yellow "  覆盖已有数据卷..." \
      || docker volume create freellmapi-data &>/dev/null
    _copy_dir_to_volume "$rbase/data" "freellmapi-data" || { red "数据卷恢复失败"; rm -rf "$rtmp"; return 1; }
    green "  数据卷已恢复"
  else
    yellow "  备份中无数据卷，跳过"
  fi
  ...
  if [[ -f "$compose_dir/docker-compose.yml" ]]; then
    ...
    docker compose up -d && cd - >/dev/null || true
    sleep 10
    if docker ps --format '{{.Names}}' | grep -q "^freellmapi$"; then
      local ip; ip=$(get_local_ip)
      local port; port=$(grep '^PORT=' "$compose_dir/.env" 2>/dev/null | cut -d= -f2 | tr -d ' ' || echo "3001")
      [[ -z "$port" ]] && port="3001"
      echo ""
      green "✅ FreeLLMAPI 恢复成功"
    ...
    yellow "未找到 docker-compose.yml，请重新部署。"
  fi
}
function freellmapi_list_backups() {
  echo ""
  cyan "FreeLLMAPI 备份列表："
  local found=false
  while IFS= read -r f; do
    local sz; sz=$(du -sh "$f" 2>/dev/null | cut -f1 || echo "?")
    printf "  📦 %-48s [%s]\n" "$f" "$sz"
...
function freellmapi_backup_menu() {
  while true; do
    ...
    yellow "⚠️  ENCRYPTION_KEY + 数据卷 必须同时迁移"
    echo "──────────────────────"
    echo "  1. 备份（KEY + 数据卷 + 配置）"
    echo "  2. 恢复（覆盖并自动启动）"
    echo "  3. 查看备份列表"
    echo "  0. 返回"
    echo ""
...
function backup_restore_menu() {
  while true; do
    ...
    cyan "备份/恢复管理"
    echo "──────────────"
    echo "  1. One-API / New-API / Horizon（绑定挂载卷）"
    echo "  2. OmniRoute（Docker 命名卷）"
    echo "  3. FreeLLMAPI（KEY + Docker 命名卷）"
    echo "  0. 返回"
    echo ""
    read -rp "选项 (0-3)：" ch </dev/tty
    case "$ch" in
      1) general_backup_menu    || true ;;
      2) omniroute_backup_menu  || true ;;
      3) freellmapi_backup_menu || true ;;
      0) return 0 ;;
      *) red "无效选项。" ;;
    esac
  done
...
function deploy_service_sqlite() {
  local cname="$1" image="$2" int_port="$3" data_name="$4"
  ...
  [[ "$OS" == "openwrt" ]] && {
    yellow "OpenWrt 存储有限，数据在 $data_dir，建议挂载外部存储。"
...
function _get_container_runtime_config() {
  local cname="$1"
  local nm; nm=$(docker inspect -f '{{.HostConfig.NetworkMode}}' "$cname" 2>/dev/null || echo "bridge")
  local pt; pt=$(docker inspect "$cname" 2>/dev/null | grep -A2 '"3000/tcp"' | grep 'HostPort' | head -n1 | grep -oE '[0-9]+' || echo "")
  local ci; ci=$(docker inspect "$cname" --format '{{.Config.Image}}' 2>/dev/null || echo "")
  echo "$nm|$pt|$ci"
}
function _upgrade_bind_service() {
  ...
  local cur_image; cur_image=$(docker inspect "$cname" --format '{{.Config.Image}}' 2>/dev/null || echo "未知")
  green "当前版本：$cur_image"
  ...
  if [[ "$new_image" == "$cur_image" ]]; then
    yellow "版本相同。"
    read -rp "强制重新拉取并重启？(y/n，默认 n)：" fr </dev/tty
    [[ ! "${fr:-n}" =~ ^[Yy]$ ]] && { yellow "已取消。"; return 0; }
  fi
  pull_image_with_retry "$new_image" || return 1
  local rc; rc=$(_get_container_runtime_config "$cname")
  local old_nm; old_nm=$(echo "$rc" | cut -d'|' -f1)
  local old_pt; old_pt=$(echo "$rc" | cut -d'|' -f2)
  yellow "停止旧容器..."
...
function deploy_one_api_sqlite() {
  ...
  cyan "镜像：ghcr.io/songquanpeng/one-api"
  echo ""
...
function deploy_one_api_mysql() {
  local cname="one-api-mysql" int_port="3000"
...
function _deploy_new_api_common() {
  ...
  echo ""
  read -rp "数据库：(1) SQLite  (2) MySQL  (3) PostgreSQL  [默认 1]：" db_mode </dev/tty
  db_mode=${db_mode:-1}
  local data_dir="$HOME/$data_name"
  local logs_dir="$HOME/${data_name}-logs"
  validate_port "$int_port" || return 1
  choose_network_mode
  pull_image_with_retry "$sel_image" || return 1
  ensure_dir_writable "$data_dir" || return 1
  ensure_dir_writable "$logs_dir" || return 1
  local cmd=(docker run -d --name "$cname" --restart unless-stopped)
  [[ "$NETWORK_MODE" == "host" ]] && cmd+=(--network host) || cmd+=(--network bridge -p "$PORT:$int_port")
  cmd+=(-v "$data_dir:/data" -v "$logs_dir:/app/logs" -e "TZ=$DEFAULT_TZ")
  cmd+=(--log-opt max-size=10m --log-opt max-file=3)
  ...
  if [[ "$db_mode" == "2" ]]; then
    ...
    [[ ! "$db_port" =~ ^[0-9]+$ ]] && { red "端口须为数字。"; return 1; }
    db_extra="${db_user}:${db_pass}@tcp(${db_host}:${db_port})/${db_name}"
    db_info="MySQL: ${db_user}@${db_host}:${db_port}/${db_name}"
    label_suffix="(MySQL)"
    cmd+=(-e "SQL_DSN=$db_extra")
  elif [[ "$db_mode" == "3" ]]; then
    yellow "PostgreSQL 连接信息："
    read -rp "  主机：" db_host </dev/tty
    read -rp "  端口（默认 5432）：" db_port </dev/tty; db_port=${db_port:-5432}
    read -rp "  用户名：" db_user </dev/tty
    read -rsp "  密码：" db_pass </dev/tty; echo
    read -rp "  库名：" db_name </dev/tty
    [[ -z "$db_host" || -z "$db_user" || -z "$db_name" ]] && { red "字段不能为空。"; return 1; }
    [[ ! "$db_port" =~ ^[0-9]+$ ]] && { red "端口须为数字。"; return 1; }
    db_extra="postgres://${db_user}:${db_pass}@${db_host}:${db_port}/${db_name}"
    db_info="PgSQL: ${db_user}@${db_host}:${db_port}/${db_name}"
    label_suffix="(PostgreSQL)"
    cmd+=(-e "SQL_DSN=$db_extra")
  fi
...
function deploy_new_api() {
  echo ""
  cyan "部署 New-API（旧版 calcium-ion）"
  cyan "镜像：ghcr.io/calcium-ion/new-api"
  cyan "支持：SQLite / MySQL / PostgreSQL + 日志目录"
  echo ""
...
function deploy_new_api_quantum() {
  ...
  cyan "镜像：calciumion/new-api（Docker Hub）"
  cyan "支持：SQLite / MySQL / PostgreSQL + 日志目录"
  echo ""
  check_existing_container "new-api" || return 1
  select_image_version "New-API QuantumNous" "$NEW_API_IMAGE_BASE_QUANTUM" "dockerhub" "calciumion/new-api" 8
  _deploy_new_api_common "new-api" "$SELECTED_IMAGE" 3000 "new-api-data" "New-API QuantumNous"
}
function deploy_new_api_horizon() {
  echo ""
  cyan "部署 New-API Horizon（高性能版）⭐ 推荐"
  cyan "镜像：calciumion/new-api-horizon（Docker Hub）"
  yellow "· CPU 占用降低约 5%，覆盖 One-API/New-API 全部功能"
  yellow "· 支持 OpenAI / Claude / Gemini 兼容接口"
  yellow "· 支持 SQLite / MySQL / PostgreSQL + 日志目录"
  echo ""
...
function deploy_omniroute() {
  local cname="omniroute"
  local int_port="20128"
  local volume_name="omniroute-data"
  echo ""
  cyan "部署 OmniRoute"
  cyan "镜像：diegosouzapw/omniroute（Docker Hub，100K+ 下载）"
  yellow "· 支持 268+ AI 提供商 / 500+ 模型，智能路由自动降级"
  yellow "· 聚合 43 个免费 Provider，免费额度实时展示"
  yellow "· 支持 OpenAI / Claude / Gemini / 嵌入 / 图像 / 视频等 10 种 API"
  yellow "· Token 压缩节省 15-95%，MCP Server，100% 本地运行，零遥测"
  yellow "· 默认端口 20128，与其他服务不冲突，可同时部署"
  echo ""
  yellow "💡 与 New-API Horizon 的关系："
  yellow "   New-API Horizon = API Key 管理分发（向多用户分发 Key、计费审计）"
  yellow "   OmniRoute       = 开发者本地 AI 网关（一个端点接所有模型、自动路由）"
  yellow "   两者功能互补，可同时部署"
  echo ""
  check_existing_container "$cname" || return 1
  select_image_version "OmniRoute" "$OMNIROUTE_IMAGE_BASE" "dockerhub" "diegosouzapw/omniroute" 8
  local sel_image="$SELECTED_IMAGE"
  local sug_port; sug_port=$(find_available_port "$int_port")
  green "建议端口：$sug_port"
  local attempts=0
  while true; do
    attempts=$((attempts + 1))
    [[ "$attempts" -gt 10 ]] && { red "端口输入错误次数过多。"; return 1; }
    read -rp "输入端口（留空使用 $sug_port）：" up </dev/tty
    PORT=${up:-$sug_port}
    if ! [[ "$PORT" =~ ^[0-9]+$ ]] || [[ "$PORT" -lt 1 || "$PORT" -gt 65535 ]]; then
      red "无效端口。"; continue
    fi
  ...
  echo ""
  echo "网络访问范围："
  echo "  1. 仅本机（127.0.0.1：安全，推荐个人开发者）"
  echo "  2. 局域网（0.0.0.0：适合局域网服务器共享访问）"
  read -rp "选项 (1-2，默认 1)：" bc </dev/tty
  [[ "${bc:-1}" == "2" ]] && { host_bind="0.0.0.0"; yellow "⚠️  局域网模式，请在可信网络使用。"; }
  green "绑定地址：$host_bind"
  pull_image_with_retry "$sel_image" || return 1
  if ! docker volume inspect "$volume_name" &>/dev/null; then
    yellow "创建数据卷 $volume_name..."
    docker volume create "$volume_name" &>/dev/null && green "  数据卷已创建" || { red "数据卷创建失败！"; return 1; }
  else
    yellow "  数据卷 $volume_name 已存在，复用。"
  fi
  local cmd=(docker run -d --name "$cname" --restart unless-stopped)
  cmd+=(--stop-timeout 40)
  cmd+=(-p "${host_bind}:${PORT}:${int_port}")
  cmd+=(-v "${volume_name}:/app/data")
  cmd+=(-e "TZ=$DEFAULT_TZ")
  cmd+=(-e "PORT=${int_port}")
  cmd+=("$sel_image")
  yellow "执行：${cmd[*]}"
  ...
  local ip; ip=$(get_local_ip)
  local display_host; [[ "$host_bind" == "0.0.0.0" ]] && display_host="$ip" || display_host="localhost"
  echo ""
  green "✅ OmniRoute 部署成功"
  green "   Dashboard  : http://${display_host}:${PORT}/dashboard"
  green "   API 端点   : http://${display_host}:${PORT}/v1/chat/completions"
  green "   模型列表   : http://${display_host}:${PORT}/v1/models"
  green "   免费额度   : http://${display_host}:${PORT}/dashboard/free-tiers"
  green "   数据卷     : $volume_name（SQLite WAL 模式）"
  yellow "   ⚠️  首次访问 Dashboard 配置 Provider API Key 后即可使用"
  yellow "   ⚠️  停止容器请用：docker stop $cname（自动等待 40 秒写入完成）"
  yellow "   建议：主菜单 → 备份/恢复 → OmniRoute 完成首次备份"
  echo ""
}
function deploy_freellmapi() {
  local cname="freellmapi" int_port="3001"
  local compose_dir="$FREELLMAPI_COMPOSE_DIR"
  echo ""
  cyan "部署 FreeLLMAPI"
  cyan "镜像：ghcr.io/tashfeenahmed/freellmapi"
  ...
  echo ""
  echo "网络访问范围："
  echo "  1. 仅本机（HOST_BIND=127.0.0.1，安全）"
  echo "  2. 局域网（HOST_BIND=0.0.0.0）"
  read -rp "选项 (1-2，默认 1)：" bc </dev/tty
  [[ "${bc:-1}" == "2" ]] && { host_bind="0.0.0.0"; yellow "⚠️  局域网模式，请在可信网络使用。"; }
  green "绑定：$host_bind"
  local rpm="120"
  ...
  if [[ -f "$key_file" ]]; then
    enc_key=$(cat "$key_file"); green "复用已有 KEY：$key_file"
  else
    enc_key=$(openssl rand -hex 32)
    echo "$enc_key" > "$key_file"; chmod 600 "$key_file"
    green "已生成新 KEY：$key_file"
    red "⚠️  请备份此文件，升级/迁移必须保留！"
  fi
  if [[ -d "$compose_dir" ]]; then
    yellow "已有部署目录：$compose_dir"
    read -rp "覆盖重新部署？(y/n，默认 n)：" ow </dev/tty
...
EOF
  chmod 600 "$compose_dir/.env"; green ".env 已写入"
...
EOF
  green "docker-compose.yml 已写入"
  pull_image_with_retry "$sel_image" || return 1
  cd "$compose_dir" || { red "无法进入目录：$compose_dir"; return 1; }
  docker compose up -d || { red "启动失败！"; cd - >/dev/null || true; return 1; }
  cd - >/dev/null || true
  yellow "等待启动（10 秒）..."
  sleep 10
  if ! docker ps --format '{{.Names}}' | grep -q "^freellmapi$"; then
  ...
  local ip; ip=$(get_local_ip)
  local url; [[ "$host_bind" == "0.0.0.0" ]] && url="http://$ip:$chosen_port" || url="http://localhost:$chosen_port"
  echo ""
  green "✅ FreeLLMAPI 部署成功"
  green "   Dashboard : $url"
  green "   API       : $url/v1/chat/completions"
  green "   数据卷    : freellmapi-data"
  green "   KEY 文件  : $key_file"
  yellow "   添加 Key 后请立即备份：主菜单 → 备份/恢复"
  yellow "   如需 Setup Code：管理菜单 → 查看 Setup Code"
  echo ""
}
function deploy_menu() {
  while true; do
    ...
    echo "  4. New-API        新版     calciumion/new-api"
    echo "  5. New-API Horizon         calciumion/new-api-horizon  ⭐"
    echo "  6. OmniRoute               diegosouzapw/omniroute      🆕"
    echo "  7. FreeLLMAPI              ghcr.io/tashfeenahmed/freellmapi"
    echo ""
    yellow "  💡 New-API Horizon = API Key 管理分发（向多用户分发、计费审计）"
    yellow "  💡 OmniRoute       = 开发者本地 AI 网关（一端点接所有模型）"
    yellow "  💡 两者功能互补，可同时部署"
    echo ""
    echo "  0. 返回"
    echo ""
    read -rp "选项 (0-7)：" ch </dev/tty
    case "$ch" in
      ...
      5) deploy_new_api_horizon || true; press_any_key ;;
      6) deploy_omniroute       || true; press_any_key ;;
      7) deploy_freellmapi      || true; press_any_key ;;
      0) return 0 ;;
      *) red "无效选项，请输入 0-7。" ;;
    esac
  done
...
function _manage_service_menu() {
  local svc_label="$1" cname="$2" int_port="$3" data_dir="$4"
  local img_base="$5" fetch_type="$6" fetch_src="$7" img_pat="$8" data_name="$9"
  while true; do
    echo ""
    cyan "管理：$svc_label"
    docker ps -a --format '{{.Names}}' | grep -q "^${cname}$" || {
      yellow "未发现容器 $cname，请先部署。"; press_any_key; return 0
    }
    docker ps -a --format "table {{.Names}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}" \
      | grep -E "NAMES|^${cname}" || true
    echo ""
    ...
    case "$mc" in
      1) docker logs -f "$cname" || true ;;
      2) docker logs --tail=100 "$cname" || true; press_any_key ;;
      3) docker stop  "$cname" && green "✅ 已停止。" || red "停止失败。"; press_any_key ;;
      4) docker start "$cname" && green "✅ 已启动。" || red "启动失败。"; press_any_key ;;
      5)
        docker restart "$cname" || true; sleep 3
        docker ps --format "  {{.Names}} {{.Status}}" | grep "^  ${cname}" || true
        press_any_key
        ;;
      6)
        _upgrade_bind_service "$svc_label" "$cname" "$int_port" "$data_dir" \
          "$img_base" "$fetch_type" "$fetch_src" || true
        press_any_key
        ;;
      7) _uninstall_bind_service "$cname" "$data_name" "$img_pat" || true; press_any_key ;;
      0) return 0 ;;
      *) red "无效选项。" ;;
    esac
  done
...
function manage_one_api() {
  ...
  if [[ ${#found_containers[@]} -eq 0 ]]; then
    yellow "未发现 One-API 容器，请先部署。"; press_any_key; return 0
  fi
  local target_cname="${found_containers[0]}"
  if [[ ${#found_containers[@]} -gt 1 ]]; then
    echo "选择操作对象："
    local idx=1
    for cn in "${found_containers[@]}"; do echo "  $idx. $cn"; idx=$((idx+1)); done
    read -rp "选项（默认 1）：" ci </dev/tty; ci=${ci:-1}
    [[ "$ci" =~ ^[0-9]+$ && "$ci" -ge 1 && "$ci" -le ${#found_containers[@]} ]] \
      && target_cname="${found_containers[$((ci-1))]}"
  fi
  local data_name="one-api-data"
  [[ "$target_cname" == "one-api-mysql" ]] && data_name="one-api-mysql-logs"
  _manage_service_menu "One-API" "$target_cname" 3000 "$HOME/$data_name" \
    "$ONE_API_IMAGE_BASE" "github" "songquanpeng/one-api" \
    "ghcr.io/songquanpeng/one-api" "$data_name"
}
function manage_new_api() {
  local cur_img; cur_img=$(docker inspect "new-api" --format '{{.Config.Image}}' 2>/dev/null || echo "")
  local use_base="$NEW_API_IMAGE_BASE_GHCR" use_type="ghcr" use_src="calcium-ion/new-api"
  echo "$cur_img" | grep -q "calciumion" && {
    use_base="$NEW_API_IMAGE_BASE_QUANTUM"; use_type="dockerhub"; use_src="calciumion/new-api"
  }
  _manage_service_menu "New-API" "new-api" 3000 "$HOME/new-api-data" \
    "$use_base" "$use_type" "$use_src" \
    "ghcr.io/calcium-ion/new-api\\|calciumion/new-api" "new-api-data"
}
function manage_new_api_horizon() {
  _manage_service_menu "New-API Horizon ⭐" "new-api-horizon" 3000 "$HOME/new-api-horizon-data" \
    "$NEW_API_HORIZON_IMAGE_BASE" "dockerhub" "calciumion/new-api-horizon" \
    "calciumion/new-api-horizon" "new-api-horizon-data"
}
function manage_omniroute() {
  ...
  while true; do
    ...
    case "$mc" in
      ...
      2) docker logs --tail=100 "$cname" || true; press_any_key ;;
      3)
        yellow "停止 OmniRoute（等待 SQLite WAL 写入，最长 40 秒）..."
        docker stop "$cname" && green "✅ 已停止。" || red "停止失败。"
        press_any_key
        ;;
      4) docker start "$cname" && green "✅ 已启动。" || red "启动失败。"; press_any_key ;;
      5)
        yellow "重启 OmniRoute（等待 SQLite WAL，最长 40 秒）..."
        docker stop "$cname" &>/dev/null || true
        sleep 2
        docker start "$cname" && green "✅ 已重启。" || red "重启失败。"
        press_any_key
        ;;
      6)
        _prompt_backup_before_action "升级" || { press_any_key; continue; }
        local cur_image; cur_image=$(docker inspect "$cname" --format '{{.Config.Image}}' 2>/dev/null || echo "未知")
        green "当前版本：$cur_image"
        select_image_version "OmniRoute" "$OMNIROUTE_IMAGE_BASE" "dockerhub" "diegosouzapw/omniroute" 8
        local new_image="$SELECTED_IMAGE"
        if [[ "$new_image" == "$cur_image" ]]; then
          yellow "版本相同。"
          read -rp "强制重新拉取并重启？(y/n，默认 n)：" fr </dev/tty
          [[ ! "${fr:-n}" =~ ^[Yy]$ ]] && { press_any_key; continue; }
        fi
        pull_image_with_retry "$new_image" || { press_any_key; continue; }
        local rc; rc=$(_get_container_runtime_config "$cname")
        local old_ports; old_ports=$(docker inspect "$cname" 2>/dev/null \
          | grep -A2 '"20128/tcp"' | grep 'HostPort' | head -n1 | grep -oE '[0-9]+' || echo "20128")
        local old_bind; old_bind=$(docker inspect "$cname" \
          --format '{{range $p, $conf := .HostConfig.PortBindings}}{{range $conf}}{{.HostIP}}{{end}}{{end}}' \
          2>/dev/null | head -n1 || echo "127.0.0.1")
        [[ -z "$old_bind" ]] && old_bind="127.0.0.1"
        yellow "停止旧容器..."
        docker stop "$cname" &>/dev/null || true
        docker rm   "$cname" &>/dev/null || true
        local cmd=(docker run -d --name "$cname" --restart unless-stopped)
        cmd+=(--stop-timeout 40)
        cmd+=(-p "${old_bind}:${old_ports}:20128")
        cmd+=(-v "${volume_name}:/app/data")
        cmd+=(-e "TZ=$DEFAULT_TZ" -e "PORT=20128")
        cmd+=("$new_image")
        yellow "执行：${cmd[*]}"
        "${cmd[@]}" && green "✅ OmniRoute 已升级到：$new_image" || red "升级后启动失败。"
        press_any_key
        ;;
      7)
        cyan "OmniRoute 数据卷信息："
        docker volume inspect "$volume_name" 2>/dev/null || yellow "未找到数据卷 $volume_name。"
        press_any_key
        ;;
      8)
        _prompt_backup_before_action "卸载" || { press_any_key; continue; }
        yellow "停止并删除容器..."
        docker stop "$cname" &>/dev/null || true
        docker rm   "$cname" &>/dev/null || true
        green "  容器已删除"
        local imgs; imgs=$(docker images --format '{{.Repository}}:{{.Tag}}' \
          | grep "^diegosouzapw/omniroute" || true)
        if [[ -n "$imgs" ]]; then
          echo "$imgs" | while read -r i; do echo "  镜像：$i"; done
          read -rp "  删除镜像？(y/n，默认 n)：" di </dev/tty
          [[ "${di:-n}" =~ ^[Yy]$ ]] && echo "$imgs" | xargs docker rmi 2>/dev/null \
            && green "  镜像已删除。" || yellow "  跳过。"
        fi
        if docker volume inspect "$volume_name" &>/dev/null; then
          red "  ⚠️  删除数据卷将永久丢失所有 OmniRoute 配置和 Provider Key！"
          read -rp "  删除数据卷 $volume_name？(y/n，默认 n)：" dv </dev/tty
          [[ "${dv:-n}" =~ ^[Yy]$ ]] && docker volume rm "$volume_name" \
            && green "  数据卷已删除。" || yellow "  已保留数据卷。"
        fi
        green "✅ OmniRoute 卸载完成"
...
function _freellmapi_upgrade_version() {
  ...
  echo ""
  cyan "FreeLLMAPI 版本升级"
  docker ps -a --format '{{.Names}}' | grep -q "^freellmapi$" || { yellow "未发现容器，请先部署。"; return 1; }
  local cur; cur=$(docker inspect freellmapi --format '{{.Config.Image}}' 2>/dev/null || echo "未知")
  green "当前版本：$cur"
  _prompt_backup_before_action "版本切换" || return 0
  select_image_version "FreeLLMAPI" "$FREELLMAPI_IMAGE_BASE" "github" "tashfeenahmed/freellmapi" 8
  local new_image="$SELECTED_IMAGE"
  if [[ "$new_image" == "$cur" ]]; then
    yellow "版本相同。"
    read -rp "强制重拉并重启？(y/n，默认 n)：" fr </dev/tty
    [[ ! "${fr:-n}" =~ ^[Yy]$ ]] && { yellow "已取消。"; return 0; }
  fi
  pull_image_with_retry "$new_image" || return 1
  if [[ -f "$compose_file" ]]; then
    ...
    cd "$compose_dir" || { red "无法进入目录。"; return 1; }
    docker compose up -d --no-deps freellmapi || true
    cd - >/dev/null || true
  else
    yellow "未找到 docker-compose.yml，请重新部署。"; return 0
  fi
  sleep 8
  if docker ps --format '{{.Names}}' | grep -q "^freellmapi$"; then
    local actual; actual=$(docker inspect freellmapi --format '{{.Config.Image}}' 2>/dev/null || echo "未知")
    green "✅ 切换成功：$actual"
  else
    red "容器启动异常："
...
function _freellmapi_show_setup_code() {
  cyan "查询 Setup Code..."
  local result; result=$(docker logs freellmapi 2>&1 | grep -iE "code|setup|one.time|one-time|pin" || true)
  if [[ -n "$result" ]]; then
    green "找到相关日志："; echo "$result"
  else
    yellow "关键词未匹配，显示启动前 80 行："
    docker logs freellmapi 2>&1 | head -80 || true
  fi
  yellow "若未找到，可重启：docker compose -f ~/freellmapi/docker-compose.yml restart"
}
function manage_freellmapi() {
  ...
  while true; do
    echo ""
    cyan "管理：FreeLLMAPI"
    docker ps -a --format '{{.Names}}' | grep -q "^freellmapi$" || {
      yellow "未发现容器，请先部署。"; press_any_key; return 0
    }
    docker ps -a --format "table {{.Names}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}" \
    ...
    echo "  6. 升级版本"
    echo "  7. 查看数据卷"
    echo "  8. 查看 Setup Code"
    echo "  9. 卸载"
    echo "  0. 返回"
    read -rp "选项 (0-9)：" mc </dev/tty; echo ""
    case "$mc" in
      1)
        $use_compose && docker compose -f "$compose_file" logs -f freellmapi \
          || docker logs -f freellmapi || true
        ;;
      2)
        $use_compose && docker compose -f "$compose_file" logs --tail=100 freellmapi \
          || docker logs --tail=100 freellmapi || true
        press_any_key
        ;;
      3)
        $use_compose && docker compose -f "$compose_file" stop freellmapi \
          || docker stop freellmapi || true
        green "✅ 已停止。"; press_any_key
        ;;
      4)
        $use_compose && docker compose -f "$compose_file" start freellmapi \
          || docker start freellmapi || true
        green "✅ 已启动。"; press_any_key
        ;;
      5)
        $use_compose && docker compose -f "$compose_file" restart freellmapi \
          || docker restart freellmapi || true
        sleep 5
        docker ps --format "  {{.Names}} {{.Status}}" | grep "freellmapi" || true
        press_any_key
        ;;
      6) _freellmapi_upgrade_version || true; press_any_key ;;
      7)
        docker volume inspect freellmapi-data &>/dev/null \
          && docker volume inspect freellmapi-data || yellow "未找到 freellmapi-data 卷。"
        press_any_key
        ;;
...
function _uninstall_bind_service() {
  local cname="$1" data_name="$2" img_pat="$3"
  local data_dir="$HOME/$data_name"
  yellow "卸载：$cname"
  ...
  if [[ -d "$data_dir" ]]; then
    ...
    if [[ "${dd:-n}" =~ ^[Yy]$ ]]; then
      read -rp "  先备份？(y/n，默认 n)：" bk </dev/tty
      local do_del=true
      [[ "${bk:-n}" =~ ^[Yy]$ ]] && {
        local bdir="${data_dir}-backup-$(date +%Y%m%d_%H%M%S)"
        cp -a "$data_dir" "$bdir" && green "  已备份：$bdir" || { red "  备份失败，取消删除。"; do_del=false; }
      }
      if $do_del; then
        local rc="rm -rf \"$data_dir\""
        [[ "$EUID" -ne 0 ]] && command -v sudo &>/dev/null && rc="sudo $rc"
        eval "$rc" && green "  数据已删除" || red "  请手动：rm -rf $data_dir"
      fi
    else
      yellow "  保留数据目录"
    fi
...
function uninstall_freellmapi() {
  ...
  red "完全卸载 FreeLLMAPI"
  red "──────────────────────"
  yellow "⚠️  强烈建议先备份（主菜单 → 备份/恢复）！"
  _prompt_backup_before_action "卸载" || return 0
  read -rp "再次确认（输入 yes 确认）：" cf </dev/tty
  [[ "$cf" != "yes" ]] && { yellow "已取消。"; return 0; }
  yellow "1/6 停止并删除容器..."
  if docker ps -a --format '{{.Names}}' | grep -q "^freellmapi$"; then
    [[ -f "$compose_file" ]] && {
      cd "$compose_dir" || true; docker compose down freellmapi 2>/dev/null || true; cd - >/dev/null || true
    }
    docker stop freellmapi &>/dev/null || true
    docker rm   freellmapi &>/dev/null || true
    green "  容器已删除"
  ...
  if [[ -n "$imgs" ]]; then
    echo "$imgs" | while read -r i; do echo "  $i"; done
    read -rp "  删除？(y/n，默认 y)：" di </dev/tty
    [[ "${di:-y}" =~ ^[Yy]$ ]] && echo "$imgs" | xargs docker rmi 2>/dev/null \
      && green "  已删除" || yellow "  跳过"
  fi
  yellow "3/6 删除数据卷..."
  if docker volume inspect freellmapi-data &>/dev/null; then
    red "  ⚠️  删除后 provider keys 和数据永久丢失！"
    read -rp "  确认删除？(y/n，默认 n)：" bv </dev/tty
    [[ "${bv:-n}" =~ ^[Yy]$ ]] \
      && docker volume rm freellmapi-data && green "  数据卷已删除" \
      || yellow "  已保留数据卷"
  fi
  yellow "4/6 删除项目目录..."
  if [[ -d "$compose_dir" ]]; then
    read -rp "  删除 $compose_dir？(y/n，默认 y)：" dd </dev/tty
    if [[ "${dd:-y}" =~ ^[Yy]$ ]]; then
  ...
  yellow "5/6 处理 ENCRYPTION_KEY..."
  if [[ -f "$key_file" ]]; then
    red "  ⚠️  此文件是密钥唯一备份！"
    read -rp "  删除？(y/n，默认 n)：" dk </dev/tty
    [[ "${dk:-n}" =~ ^[Yy]$ ]] && rm -f "$key_file" && green "  KEY 已删除" || yellow "  已保留：$key_file"
  fi
  yellow "6/6 清理悬空镜像..."
  local dangling; dangling=$(docker images -f "dangling=true" -q 2>/dev/null || true)
  if [[ -n "$dangling" ]]; then
    local cnt; cnt=$(echo "$dangling" | wc -l)
    read -rp "  发现 ${cnt} 个悬空镜像，清理？(y/n，默认 n)：" cd2 </dev/tty
    [[ "${cd2:-n}" =~ ^[Yy]$ ]] && docker image prune -f && green "  已清理" || yellow "  跳过"
  else
    green "  无悬空镜像"
  fi
  echo ""; green "✅ FreeLLMAPI 已完全卸载"
}
function view_container_status() {
  cyan "当前 Docker 容器："
  [[ $(docker ps -a --format '{{.Names}}' | wc -l) -eq 0 ]] \
    && yellow "  无任何容器。" \
    || docker ps -a --format "table {{.Names}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}"
  echo ""
  cyan "Docker 磁盘占用："
...
function main_menu() {
  ...
  _net_detect
  if [[ "$OS" == "openwrt" ]]; then
    yellow "OpenWrt 提示：Bash 未装请先 opkg install bash，存储有限请挂外部存储。"
    sleep 1
  fi
  while true; do
    local dv="未安装" cv="未安装"
    ...
    docker compose version &>/dev/null \
      ...
      || { command -v docker-compose &>/dev/null && cv=$(docker-compose --version | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -n1); }
    local s1; s1=$(_svc_status_tag "one-api")
    local s2; s2=$(_svc_status_tag "one-api-mysql")
    local s3; s3=$(_svc_status_tag "new-api")
    local s4; s4=$(_svc_status_tag "new-api-horizon")
    local s5; s5=$(_svc_status_tag "omniroute")
    local s6; s6=$(_svc_status_tag "freellmapi")
    ...
    echo ""
    cyan "─── Docker 服务管理 ───────────────────────────────"
    echo "  系统: $OS  架构: $ARCH  Docker: $dv  Compose: $cv"
    ...
    echo "  2. 管理 One-API"
    echo "  3. 管理 New-API"
    echo "  4. 管理 New-API Horizon  ⭐"
    echo "  5. 管理 OmniRoute        🆕"
    echo "  6. 管理 FreeLLMAPI"
    echo "  7. 查看容器状态"
    echo "  8. 备份/恢复"
    echo "  9. 镜像加速器配置"
    echo "  0. 退出"
    ...
    case "$ch" in
      ...
      7) view_container_status   || true; press_any_key ;;
      8) backup_restore_menu     || true ;;
      9) configure_docker_mirror || true ;;
      0) green "感谢使用，退出。"; exit 0 ;;
      *) red "无效选项，请输入 0-9。" ;;
    esac
  done
}
