...
mihomo_management_menu() {
    while true; do 
        printf "\n%b=== Mihomo 管理菜单 ===%b\n" "$GREEN" "$NC"
        printf "  1) 部署/更新 Mihomo 核心 (MetaCubeX)\n"
        printf "  2) 部署 Mihomo Alpha with Smart Group (vernesong)\n" # 新增选项
        printf "  3) 配置订阅和模式\n"
        printf "  4) 更新配置并立即运行\n"
        printf "  5) 设置/管理自动更新\n"
        printf "  6) 管理服务 (启停等)\n"
        ...
        case "$choice" in
            1) install_mihomo ;; 
            2) install_mihomo_alpha_smart ;; # 调用新的安装函数
            3) setup_service_env "$MH_ENV_FILE" "Mihomo" "global/gfwlist/rule/direct" ;; 
            4) update_config_and_run_internal "mihomo" ;;
            5) setup_scheduled_update_internal "mihomo" ;;
            6) service_management_menu "mihomo" ;;
            7) uninstall_service "mihomo" ;;
            q|Q) green "返回主菜单。"; return 0 ;; 
            *) red "无效选项 '$choice'" ;;
        esac
        printf "\n%b按 [Enter] 键继续...%b" "$YELLOW" "$NC"
