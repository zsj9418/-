...
# ── GitHub 代理前缀（用于 docker-compose 下载） ──────────────
...
SPEED_TEST_RESULTS=()
BEST_DOCKER_MIRROR=""       # 由 detect_best_docker_mirror() 填充
...
install_dependencies() {
  ...
  if [[ ${#missing_req_deps[@]} -eq 0 && ${#missing_opt_deps[@]} -eq 0 ]]; then
    echo -e "${GREEN}所有依赖均已安装。${NC}"; return 0
  fi
  case "$PKG_MANAGER" in
    apt-get)
...
# 测速所有 Docker 二进制镜像源，将最快的写入 BEST_DOCKER_MIRROR
detect_best_docker_mirror() {
  section "Docker 下载源测速"
  echo -e "  正在探测各下载源延迟，请稍候...\n"

  local best_ms=9999 best_mirror=""
  local probe_path="/${ARCH}/docker-${RECOMMENDED_LEGACY_DOCKER_VERSION}.tgz"
  # 只探测 HEAD 请求，避免实际下载
  for mirror in "${DOCKER_BINARY_MIRRORS[@]}"; do
    local probe_url="${mirror}${probe_path}"
    local ms
    ms=$(curl -o /dev/null -s -w "%{time_total}" \
      --head --connect-timeout 6 --max-time 8 "$probe_url" 2>/dev/null \
      | tail -1 | awk '{printf "%.0f", $1*1000}')
    # curl -w 输出到 stdout 而非 pipe，用另一种写法
    ms=$(curl -o /dev/null -s -w "%{time_total}" \
      -I --connect-timeout 6 --max-time 8 "$probe_url" 2>/dev/null || echo "9.999")
    ms=$(awk "BEGIN{printf \"%.0f\", ${ms} * 1000}" 2>/dev/null || echo 9999)

    if (( ms >= 8000 )); then
      printf "  ${RED}✗${NC}  %-55s (不可达)\n" "$mirror"
    else
      printf "  ${GREEN}✓${NC}  %-55s %sms\n" "$mirror" "$ms"
      if (( ms < best_ms )); then
        best_ms=$ms
        best_mirror="$mirror"
      fi
  ...
  if [[ -z "$best_mirror" ]]; then
    echo -e "  ${RED}警告：所有下载源均不可达，将使用官方源（可能很慢）。${NC}"
    BEST_DOCKER_MIRROR="${DOCKER_BINARY_MIRRORS[0]}"
  else
    echo -e "  ${GREEN}最快下载源: ${best_mirror} (${best_ms}ms)${NC}"
    BEST_DOCKER_MIRROR="$best_mirror"
  fi
}

# 通用下载函数
# 用法: download_with_fallback <type> <url_suffix_or_full_url> <dest>
#   type = "docker"   → 遍历 DOCKER_BINARY_MIRRORS，url_suffix 为 /ARCH/docker-VER.tgz
#   type = "github"   → 遍历 GITHUB_PROXY_PREFIXES，url 为完整 GitHub URL
#   type = "generic"  → 直接下载完整 URL，无代理
download_with_fallback() {
  ...
  _do_curl() {
    ...
    curl -fL --retry 2 --connect-timeout 15 --max-time 300 \
      --progress-bar "$u" -o "$d" 2>&1
    return ${PIPESTATUS[0]}
  }

  case "$type" in
    docker)
      # url 此处为路径后缀，如 /x86_64/docker-29.7.1.tgz
      local suffix="$url"
      # 优先用测速选出的最快源
      local mirrors=()
      ...
      for mirror in "${mirrors[@]}"; do
        ...
        echo -e "\n  ${CYAN}尝试: ${mirror}${NC}" >&2
        if _do_curl "$full_url" "$dest"; then
          echo -e "  ${GREEN}下载成功 ← ${mirror}${NC}" >&2
          return 0
        fi
        echo -e "  ${YELLOW}失败，切换下一源...${NC}" >&2
      done
    ...
    github)
      # 先直连
      echo -e "\n  ${CYAN}直连 GitHub...${NC}" >&2
      if _do_curl "$url" "$dest"; then
        echo -e "  ${GREEN}直连下载成功${NC}" >&2; return 0
      fi
      echo -e "  ${YELLOW}直连失败，尝试代理...${NC}" >&2
      for prefix in "${GITHUB_PROXY_PREFIXES[@]}"; do
        local proxy_url="${prefix}${url}"
        echo -e "\n  ${CYAN}代理: ${prefix}${NC}" >&2
        if _do_curl "$proxy_url" "$dest"; then
          echo -e "  ${GREEN}代理下载成功${NC}" >&2; return 0
        fi
...
check_docker_installed() {
  if command -v docker >/dev/null 2>&1; then
    echo -e "${GREEN}Docker 已安装：$(docker --version)${NC}"; return 0
  fi
  echo -e "${YELLOW}Docker 未安装。${NC}"; return 1
}

check_docker_compose_installed() {
  if command -v docker-compose >/dev/null 2>&1; then
    echo -e "${GREEN}docker-compose 已安装：$(docker-compose --version)${NC}"; return 0
  fi
  if docker compose version >/dev/null 2>&1; then
    echo -e "${GREEN}Docker Compose 插件已安装：$(docker compose version | head -n1)${NC}"; return 0
  fi
  echo -e "${YELLOW}Docker Compose 未安装。${NC}"; return 1
}
...
configure_proxy_mirror() {
  ...
  if [[ -z "$input" ]]; then
    PROXY_MIRROR_URL=""; echo -e "  ${GREEN}已清空代理镜像源配置。${NC}"; return 0
  fi
  if ! [[ "$input" =~ ^https?:// ]]; then
    echo -e "  ${RED}格式不正确：必须以 http:// 或 https:// 开头。${NC}"; return 1
  fi
...
run_mirror_speed_test() {
  ...
  if (( best_ms >= 8000 )); then
    echo -e "  ${RED}所有源均不可达，请检查网络连接。${NC}"; return 1
  fi
...
_apply_speed_test_to_daemon() {
  ...
  if [[ -n "$PROXY_MIRROR_URL" ]]; then
    local exists="false"
    for m in "${reachable_mirrors[@]}"; do [[ "$m" == "$PROXY_MIRROR_URL" ]] && exists="true" && break; done
    [[ "$exists" == "false" ]] && reachable_mirrors+=("$PROXY_MIRROR_URL")
  fi
  if [[ ${#reachable_mirrors[@]} -eq 0 ]]; then
    echo -e "  ${RED}没有可达的镜像源，daemon.json 未修改。${NC}"; return
  fi
  backup_daemon_json
  local mirrors_json
...
_detect_virt_env() {
  ...
  if [[ -f /proc/user_beancounters ]] || grep -qi 'openvz\|virtuozzo' /proc/version 2>/dev/null; then
    IS_OPENVZ="true"; echo -e "  ${YELLOW}⚠${NC}  检测到 OpenVZ/Virtuozzo 虚拟化环境"; return
  fi
  if grep -qa 'lxc' /proc/1/environ 2>/dev/null || \
    ...
    IS_LXC="true"; echo -e "  ${CYAN}[INFO]${NC} 检测到 LXC 容器环境"; return
  fi
  if grep -qa 'docker\|podman' /proc/1/environ 2>/dev/null || [[ -f /.dockerenv ]]; then
    echo -e "  ${YELLOW}⚠${NC}  检测到 Docker-in-Docker 环境，部分功能受限"; return
  fi
...
_check_apparmor() {
  ...
  if [[ ! -r /sys/kernel/security/apparmor/profiles ]]; then
    ...
    APPARMOR_RESTRICTED="true"; return 0
  fi
...
ensure_docker_prereqs() {
  ...
  if [[ "$IS_OPENVZ" == "true" ]]; then
    ...
    FORCE_LEGACY_DOCKER="true"; compat_issues+=("OpenVZ 环境，降级至兼容版本")
  fi
  ...
  if [[ "$backend" == "nft" ]]; then
    ...
    if [[ "$backend" == "none" ]]; then
      compat_issues+=("iptables 完全不可用"); iptables_ok="false"
    else
  ...
  if command -v ipset >/dev/null 2>&1 && ipset list >/dev/null 2>&1; then
    echo -e "  ${GREEN}✓${NC}  ipset 可用"; ipset_ok="true"
  else
    ...
    _check_module "ip_set_hash_net" 2>/dev/null || true
    if command -v ipset >/dev/null 2>&1 && ipset list >/dev/null 2>&1; then
      echo -e "  ${GREEN}✓${NC}  ipset 安装并验证成功"; ipset_ok="true"
    else
  ...
  if _check_ipv6_available; then
    echo -e "  ${GREEN}✓${NC}  IPv6 可用"; IPV6_AVAILABLE="true"
  else
    echo -e "  ${YELLOW}⚠${NC}  IPv6 不可用，daemon.json 将设置 ip6tables: false"; IPV6_AVAILABLE="false"
  fi
  _check_apparmor
  ...
  {
    echo "overlay"; echo "br_netfilter"
    if [[ "$FORCE_LEGACY_DOCKER" == "false" ]]; then
      echo "ip_set"; echo "ip_set_hash_net"
...
select_version() {
  ...
  if command -v fzf >/dev/null 2>&1; then
    ...
    if [[ -n "$SELECTED" ]]; then echo "$SELECTED"; return
    else echo -e "  ${YELLOW}未选择，使用最新版本...${NC}" >&2; echo "${VERSIONS[0]}"; return; fi
  fi
  echo "  可用版本列表:" >&2
  local i=1
  for v in "${VERSIONS[@]}"; do printf "  %3d) %s\n" "$i" "$v" >&2; ((i++)); done
  read -r -p "  请输入序号 (回车默认选 1 最新版): " REPLY
  if [[ -z "$REPLY" || "$REPLY" == "1" ]]; then echo "${VERSIONS[0]}"
  elif [[ "$REPLY" =~ ^[0-9]+$ ]] && (( REPLY >= 1 && REPLY <= ${#VERSIONS[@]} )); then
    echo "${VERSIONS[$((REPLY-1))]}"
  else
    echo -e "  ${YELLOW}无效选择，使用最新版本...${NC}" >&2; echo "${VERSIONS[0]}"
  fi
}
...
restore_daemon_json() {
  ...
  if [[ ${#backups[@]} -eq 0 ]]; then echo -e "  ${YELLOW}暂无备份文件。${NC}"; return; fi
  local i=1
  for f in "${backups[@]}"; do
  ...
  while true; do
    ...
    case "$DCHOICE" in
      ...
      2)
        ...
        if [[ ${#backups[@]} -eq 0 ]]; then echo -e "  ${YELLOW}暂无备份。${NC}"
        else for f in "${backups[@]}"; do
          echo "  - $(basename "$f")  ($(stat -c %y "$f" 2>/dev/null | cut -d. -f1))"; done; fi
        ;;
...
show_docker_status() {
  ...
  svc_enabled=$(systemctl is-enabled docker 2>/dev/null || echo "disabled")
  [[ "$svc_status" == "active" ]] \
    && echo -e "  服务: ${GREEN}● 运行中${NC}  开机自启: $(
        [[ "$svc_enabled" == "enabled" ]] && echo "${GREEN}已开启${NC}" || echo "${YELLOW}${svc_enabled}${NC}")" \
  ...
  if [[ -f /etc/docker/daemon.json ]]; then
    if jq . /etc/docker/daemon.json >/dev/null 2>&1; then
      ...
      else echo -e "  镜像加速: ${YELLOW}未配置${NC}"; fi
    ...
    else echo -e "  状态: ${RED}存在，但 JSON 格式错误！${NC}"; fi
  else echo -e "  状态: ${YELLOW}daemon.json 不存在${NC}"; fi
...
manage_logs() {
  while true; do
    ...
    case "$LCHOICE" in
      1) [[ ! -f "$LOG_FILE" ]] && { echo -e "  ${YELLOW}日志文件不存在。${NC}"; continue; }
         tail -n 50 "$LOG_FILE" ;;
      2)
        local backups=()
        while IFS= read -r -d '' f; do backups+=("$f")
        done < <(find "$LOG_DIR" -maxdepth 1 -name "*.gz" -print0 2>/dev/null | sort -z)
        if [[ ${#backups[@]} -eq 0 ]]; then echo -e "  ${YELLOW}暂无历史备份。${NC}"
        else
          local i=1
          for f in "${backups[@]}"; do
            printf "  %2d) %-45s %s\n" "$i" "$(basename "$f")" "$(_human_size "$f")"; ((i++))
          done
          read -r -p "  输入编号查看内容（回车跳过）: " BIDX
          if [[ "$BIDX" =~ ^[0-9]+$ ]] && (( BIDX >= 1 && BIDX <= ${#backups[@]} )); then
            zcat "${backups[$((BIDX-1))]}" 2>/dev/null | tail -n 50; fi
        fi ;;
      3) local old_max=$LOG_MAX_SIZE_MB; LOG_MAX_SIZE_MB=0; rotate_log; LOG_MAX_SIZE_MB=$old_max
         echo -e "  ${GREEN}手动轮转完成。${NC}" ;;
      4) read -r -p "  确认清空当前日志？(y/n): " CONFIRM
         if [[ "$CONFIRM" == "y" ]]; then
           : > "$LOG_FILE"
           echo "[$(date '+%Y-%m-%d %H:%M:%S')] 日志已手动清空。" >> "$LOG_FILE"
           echo -e "  ${GREEN}已清空。${NC}"; fi ;;
      5) local gz_files=()
         while IFS= read -r -d '' f; do gz_files+=("$f")
         done < <(find "$LOG_DIR" -maxdepth 1 -name "*.gz" -print0 2>/dev/null)
         if [[ ${#gz_files[@]} -eq 0 ]]; then echo -e "  ${YELLOW}没有历史备份。${NC}"
         else
           read -r -p "  确认删除 ${#gz_files[@]} 个历史备份？(y/n): " CONFIRM
           [[ "$CONFIRM" == "y" ]] && rm -f "${gz_files[@]}" && echo -e "  ${GREEN}已删除。${NC}"; fi ;;
      6) read -r -p "  确认删除全部日志？不可恢复 (y/n): " CONFIRM
         if [[ "$CONFIRM" == "y" ]]; then
           find "$LOG_DIR" -maxdepth 1 -name "*.gz" -delete 2>/dev/null
           : > "$LOG_FILE"
           echo "[$(date '+%Y-%m-%d %H:%M:%S')] 全部日志已清除。" >> "$LOG_FILE"
           echo -e "  ${GREEN}已清除。${NC}"; fi ;;
      7) echo -e "  ${CYAN}当前: 单文件上限 ${LOG_MAX_SIZE_MB}MB，保留 ${LOG_MAX_BACKUPS} 个${NC}"
...
generate_daemon_config() {
  ...
  if [[ "$DO_SPEEDTEST" == "y" ]]; then run_mirror_speed_test; return; fi
  local mirrors=("${REGISTRY_MIRRORS_DEFAULT[@]}")
  if [[ -n "$PROXY_MIRROR_URL" ]]; then
    local exists="false"
    for m in "${mirrors[@]}"; do [[ "$m" == "$PROXY_MIRROR_URL" ]] && exists="true" && break; done
  ...
  [[ "$secopt_json" != "null" ]] && \
    DAEMON_CONFIG=$(echo "$DAEMON_CONFIG" | jq --argjson secopt "$secopt_json" '. + {"default-security-opt": $secopt}')
  echo -e "\n  daemon.json 内容："
