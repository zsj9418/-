...
if "agents" in cfg and isinstance(cfg["agents"], dict):
    ...
    for bad_agent_key in list(agents.keys()):
        if bad_agent_key not in ["defaults", "list"]:
            del agents[bad_agent_key]
            changed.append(f"删除无效 agents.{bad_agent_key}")
    ...
    if "defaults" in agents and isinstance(agents["defaults"], dict):
        ...
        if "model" in defaults:
            ...
            if isinstance(model_cfg, dict):
                ...
                changed.append("agents.defaults.model 转换为对象")
        if not defaults:
            del agents["defaults"]
            changed.append("删除空 agents.defaults")
    
    if not agents:
        del cfg["agents"]
        changed.append("删除空 agents")
...
configure_custom_api() {
    ...
    if $started; then
        ...
        echo ""
        echo -e "${YELLOW}💡 尝试运行: openclaw doctor --fix${NC}"
    fi
    echo ""
...
if "agents" in config:
    ...
    if isinstance(agents, dict):
        for bad_key in list(agents.keys()):
    ...
    case "$action" in
        start)
            ...
            if curl -s --max-time 3 "http://127.0.0.1:${OPENCLAW_PORT}" &>/dev/null; then
                ...
                echo -e "  ${BOLD}3)${NC} 运行: ${YELLOW}openclaw doctor --fix${NC}"
            fi
            ;;
        stop)
            confirm "确认停止?" && {
...
" 2>/dev/null || echo "ERROR")
    ...
    if is_openclaw_installed && json_is_valid; then
        ...
        if echo "$schema_out" | grep -qiE "Invalid input"; then
            ...
            confirm "  自动修复 (doctor --fix)?" && { openclaw_cmd doctor --fix >> "$LOG_FILE" 2>&1 || true; sanitize_config_for_schema | sed 's/^/    /'; ((fixed++)); }
        else
            _pass "通过"
        fi
    else
...
main_menu() {
    while true; do
        ...
        case "$choice" in
            ...
            24) sanitize_config_for_schema | sed 's/^/  /'; openclaw_cmd doctor --fix >> "$LOG_FILE" 2>&1 || true; msg_ok "已清理"; wait_and_return 3 ;;
            25)
                if is_docker_mode; then
