...
|| echo "tok$(date +%s%N | sha256sum | head -c 40)"
}
get_local_ip() {
local ip=""
...
| grep 'inet6' \
| grep -v 'temporary\|deprecated\|tentative\|fe80' \
| awk '{print $2}' \
| cut -d'/' -f1 \
| grep -v '^::1$' \
| head -1 || true)
fi
[[ -z "$ip6" ]] && ip6=$(ifconfig 2>/dev/null \
| grep 'inet6' \
| grep -v 'fe80\|::1\|temporary\|deprecated' \
| awk '{print $2}' \
| sed 's/%.*$//' \
| head -1 || true)
echo "${ip6:-}"
...
is_lan_url() {
local url="$1"
[[ "$url" =~ ^https?://(192\.168\.|10\.|172\.(1[6-9]|2[0-9]|3[0-1])\.|127\.|localhost|\[::1\]|\[fc|\[fd) ]]
}
is_ipv6_url() {
local url="$1"
[[ "$url" =~ ^https?://\[ ]]
...
_detect_deploy_mode() {
if _container_exists "$DOCKER_CONTAINER_ZH" 2>/dev/null; then
echo "Docker(中文版)"
elif _container_exists "$DOCKER_CONTAINER" 2>/dev/null; then
echo "Docker(官方版)"
...
is_docker_mode() {
! has_cmd openclaw && { _container_exists "$DOCKER_CONTAINER_ZH" 2>/dev/null || _container_exists "$DOCKER_CONTAINER" 2>/dev/null; }
}
...
if _container_running "$DOCKER_CONTAINER_ZH" 2>/dev/null; then
echo "$DOCKER_CONTAINER_ZH"
elif _container_running "$DOCKER_CONTAINER" 2>/dev/null; then
...
local tmp_env=""
tmp_env=$(mktemp 2>/dev/null || echo "/tmp/oc_env_$$")
python3 - "$cfg" "$tmp_env" << 'PYEOF' 2>/dev/null
...
for p in ["anthropic","openai","google","deepseek","groq","mistral"]:
    ...
    key = pc.get("apiKey","")
    if key:
        lines.append(f'G_API_KEYS[{p}]={shlex.quote(key)}')
        if not first_builtin: first_builtin = p
    ms = pc.get("models") or pc.get("model","")
    if ms: lines.append(f'G_API_MODELS[{p}]={shlex.quote(str(ms))}')
first_custom = ""
for name, pc in c.get("models",{}).get("providers",{}).items():
    ...
    if bu: lines.append(f'G_API_URLS[{name}]={shlex.quote(bu)}')
    if ak: lines.append(f'G_API_KEYS[{name}]={shlex.quote(ak)}')
    if at: lines.append(f'G_API_TYPES[{name}]={shlex.quote(at)}')
    if ids: lines.append(f'G_API_MODELS[{name}]={shlex.quote(ids)}')
    if not first_custom: first_custom = name
if first_custom: lines.append(f'G_DEFAULT_PROVIDER={shlex.quote(first_custom)}')
elif first_builtin: lines.append(f'G_DEFAULT_PROVIDER={shlex.quote(first_builtin)}')
...
printf -v "$varname" '%s' "$existing"; msg_ok "保留现有令牌"
elif [[ -z "$user_tok" ]]; then
printf -v "$varname" '%s' "$(_gen_token)"; msg_ok "已自动生成令牌"
elif [[ ${#user_tok} -lt 16 ]]; then
msg_warn "令牌过短(${#user_tok}字符),已自动生成"
printf -v "$varname" '%s' "$(_gen_token)"
else
printf -v "$varname" '%s' "$user_tok"; msg_ok "使用自定义令牌(长度:${#user_tok})"
fi
}
print_sysinfo() {
...
echo -e "  ${BOLD}本机IPv4${NC}    : $(get_local_ip)"
[[ -n "$ipv6" ]] && echo -e "  ${BOLD}本机IPv6${NC}    : ${ipv6}"
...
"http://[::1]:${OPENCLAW_PORT}"
"http://[::1]:${OPENCLAW_PORT}/health"
)
for url in "${urls[@]}"; do
local code=""
...
has_cmd nc && nc -6 -z -w 2 ::1 "$OPENCLAW_PORT" 2>/dev/null && return 0
return 1
}
wait_gateway_ready() {
local timeout="${1:-30}"
...
echo -e "  ${BOLD}2)${NC} host ${DIM}(共享宿主机网络,自动支持IPv6)${NC}"
echo -ne "\n${BOLD}选择[1-2](默认:1): ${NC}"
local nm=""
read_input nm "1"
...
_ask_ipv6_enabled() {
echo -e "\n${CYAN}${BOLD}── IPv6 支持 ──${NC}"
local has_v6=""
has_v6=$(get_local_ipv6)
if [[ -n "$has_v6" ]]; then
echo -e "  检测到本机IPv6: ${CYAN}${has_v6}${NC}"
echo -e "  ${BOLD}1)${NC} 启用IPv6双栈 ${GREEN}(推荐)${NC}"
echo -e "  ${BOLD}2)${NC} 仅IPv4"
echo -ne "\n${BOLD}选择[1-2](默认:1): ${NC}"
local ans=""
read_input ans "1"
[[ "$ans" == "2" ]] && echo "false" || echo "true"
else
echo -e "  ${DIM}未检测到全局IPv6地址,跳过${NC}"
echo "false"
fi
}
_docker_show_result() {
...
if [[ -n "$local_ipv6" ]] && [[ "$ipv6_enabled" == "true" || "$network_mode" == "host" ]]; then
echo -e "  ${BOLD}IPv6:${NC}     ${CYAN}http://[${local_ipv6}]:${display_port}${NC}"
fi
...
local skip_cname="$1"
local port="$2"
for cn in "$DOCKER_CONTAINER" "$DOCKER_CONTAINER_ZH"; do
[[ "$cn" == "$skip_cname" ]] && continue
if _container_running "$cn" 2>/dev/null; then
local bound=""
bound=$(docker inspect --format='{{json .HostConfig.PortBindings}}' "$cn" 2>/dev/null \
| grep -o "\"${port}/tcp\"" || true)
if [[ -n "$bound" ]]; then
msg_warn "容器 ${cn} 已占用端口 ${port}"; return 1
fi
fi
done
return 0
...
run_cmd+=(-p "0.0.0.0:${port}:18789" -p "[::]:${port}:18789")
else
run_cmd+=(-p "${port}:18789")
fi
...
docker logs --tail 30 "$cname" 2>&1 | sed 's/^/  /'; print_line
return 1
fi
msg_ok "容器已启动"
wait_gateway_ready 30
_docker_show_result "$cname" "$port" "$gw_token" "$network_mode" "$edition" "$ipv6_enabled"
log "Docker ${edition} deployed: image=${image} net=${network_mode} ipv6=${ipv6_enabled}"
else
msg_fail "容器启动失败"
docker logs --tail 20 "$cname" 2>&1 | sed 's/^/  /' || true
return 1
fi
}
deploy_docker() {
...
if _container_exists "$DOCKER_CONTAINER" 2>/dev/null; then
...
[[ "$network_mode" != "host" ]] && ipv6_enabled=$(_ask_ipv6_enabled)
local gw_token=""
prompt_gateway_token gw_token ""
_docker_check_port_conflict "$DOCKER_CONTAINER" "$port" || {
msg_warn "请先停止占用端口的容器再部署"; wait_and_return 3; return 1
}
...
msg_warn "主镜像失败,尝试备用..."
local pulled=false
for fb in "$DOCKER_IMAGE_DR34M:latest" "$DOCKER_IMAGE_1PANEL:latest" \
"$DOCKER_IMAGE_ALPINE:latest" "$DOCKER_IMAGE_MIRROR:latest"; do
[[ "$fb" == "${image}" ]] && continue
if docker pull "$fb" 2>&1 | tail -2; then image="$fb"; pulled=true; break; fi
done
$pulled || { msg_fail "镜像拉取失败"; return 1; }
fi
msg_ok "镜像就绪: ${image}"
msg_step "初始化配置..."
_docker_apply_config "$vol" "$image" "$gw_token"
_docker_start_container "$DOCKER_CONTAINER" "$vol" "$data_dir" \
"$port" "$network_mode" "$ipv6_enabled" "$gw_token" "$image" "官方版"
}
_deploy_docker_zh() {
if _container_exists "$DOCKER_CONTAINER_ZH" 2>/dev/null; then
...
local gw_token=""
prompt_gateway_token gw_token ""
_docker_check_port_conflict "$DOCKER_CONTAINER_ZH" "$port" || {
msg_warn "请先停止占用端口的容器再部署"; wait_and_return 3; return 1
}
...
msg_step "补充关键配置参数..."
_docker_apply_config "$vol" "$image" "$gw_token"
_docker_start_container "$DOCKER_CONTAINER_ZH" "$vol" "$data_dir" \
"$port" "$network_mode" "$ipv6_enabled" "$gw_token" "$image" "中文汉化版"
}
...
_container_exists "$cn" 2>/dev/null && docker start "$cn" >/dev/null 2>&1 || true
done
return 0
fi
has_cmd openclaw || return 1
ensure_minimal_config
_graceful_stop_process "openclaw.*gateway" 8
...
if [[ -z "$port_pid" ]] && has_cmd ss; then
port_pid=$(ss -lntp 2>/dev/null | grep ":$OPENCLAW_PORT" | grep -oP 'pid=\K[0-9]+' | head -1 || true)
fi
if [[ -n "$port_pid" ]]; then
...
if is_docker_mode; then
for cn in "$DOCKER_CONTAINER_ZH" "$DOCKER_CONTAINER"; do
_container_running "$cn" 2>/dev/null && docker stop "$cn" >/dev/null 2>&1 || true
done
return 0
fi
case "$SERVICE_MANAGER" in
...
_graceful_stop_process "openclaw.*gateway" 5
}
...
if is_docker_mode; then
for cn in "$DOCKER_CONTAINER_ZH" "$DOCKER_CONTAINER"; do
_container_exists "$cn" 2>/dev/null || continue
...
if $docker_mode; then
for cn in "$DOCKER_CONTAINER_ZH" "$DOCKER_CONTAINER"; do
_container_exists "$cn" 2>/dev/null && docker restart "$cn" >/dev/null 2>&1 || true
done
sleep 5; wait_gateway_ready 20
else
service_stop; sleep 2; service_start; wait_gateway_ready 25
fi
show_dashboard_info ;;
status)
msg_step "状态:"
...
local port_info=""
port_info=$(docker inspect --format='{{json .HostConfig.PortBindings}}' "$cname" 2>/dev/null || echo "")
echo "$port_info" | grep -q '"HostIp":"::' && ipv6_active=true || true
fi
fi
else
ss -lntp 2>/dev/null | grep ":${OPENCLAW_PORT}" | grep -q ':::' && ipv6_active=true || true
fi
fi
echo -e "\n${GREEN}${BOLD}╔══════════════════════════════════════════════════════════╗${NC}"
...
if [[ -n "$local_ipv6" ]]; then
if $ipv6_active; then
echo -e "${GREEN}${BOLD}║${NC}  ${BOLD}IPv6:${NC}    ${CYAN}http://[${local_ipv6}]:${OPENCLAW_PORT}${NC} ${GREEN}(已启用)${NC}"
else
echo -e "${GREEN}${BOLD}║${NC}  ${BOLD}IPv6:${NC}    ${DIM}未启用(部署时选择IPv6双栈)${NC}"
fi
fi
...
G_API_URLS["$pname"]="$final_url"
G_API_KEYS["$pname"]="$api_key"
G_API_TYPES["$pname"]="$api_type"
G_API_MODELS["$pname"]="$detected_models"
...
msg_step "重启..."
service_restart; wait_gateway_ready 15
fi
...
echo -e "  ${BOLD}IPv4:${NC}  ${CYAN}http://${local_ip}:${OPENCLAW_PORT}${NC}"
[[ -n "$local_ipv6" ]] && echo -e "  ${BOLD}IPv6:${NC}  ${CYAN}http://[${local_ipv6}]:${OPENCLAW_PORT}${NC} ${DIM}(需容器启用IPv6绑定)${NC}"
echo -e "  ${BOLD}令牌:${NC}  ${YELLOW}${gw_token}${NC}"
print_line
wait_and_return 5; return 0
fi
...
echo -e "\n${GREEN}局域网访问信息${NC}"; print_line
echo -e "  ${BOLD}IPv4:${NC}  ${CYAN}http://${local_ip}:${OPENCLAW_PORT}${NC}"
[[ -n "$local_ipv6" ]] && echo -e "  ${BOLD}IPv6:${NC}  ${CYAN}http://[${local_ipv6}]:${OPENCLAW_PORT}${NC}"
echo -e "  ${BOLD}令牌:${NC}  ${YELLOW}${gw_token}${NC}"
print_line
wait_and_return 5
}
...
echo -e "\n  IPv4访问: ${CYAN}http://${local_ip}:${OPENCLAW_PORT}${NC}  (连接时填入令牌)"
[[ -n "$local_ipv6" ]] && echo -e "  IPv6访问: ${CYAN}http://[${local_ipv6}]:${OPENCLAW_PORT}${NC}"
else
...
sync_agent_auth "$p" "$url" "${G_API_KEYS[$p]}" "${G_API_TYPES[$p]:-openai-completions}" "$agent_name" \
&& ok=$(( ok + 1 )) || fail=$(( fail + 1 ))
done
if $inject_openai && [[ -n "$first_custom" ]]; then
local url="${G_API_URLS[$first_custom]}"
is_docker_mode && is_lan_url "$url" && url=$(convert_url_for_docker "$url")
sync_agent_auth "openai" "$url" "${G_API_KEYS[$first_custom]}" "${G_API_TYPES[$first_custom]:-openai-completions}" "$agent_name" \
&& ok=$(( ok + 1 )) || fail=$(( fail + 1 ))
fi
for p in anthropic openai google deepseek groq mistral; do
[[ -z "${G_API_KEYS[$p]:-}" ]] && continue
...
sync_agent_auth "$p" "$built_url" "${G_API_KEYS[$p]}" "openai-completions" "$agent_name" \
&& ok=$(( ok + 1 )) || fail=$(( fail + 1 ))
done
msg_step "重启Gateway..."
service_restart; wait_gateway_ready 15
...
echo -ne "  [1/10] OpenClaw安装... "
is_openclaw_installed && echo -e "${GREEN}${OK}${NC}" || { echo -e "${RED}${FAIL} 未安装${NC}"; issues=$(( issues + 1 )); }
echo -ne "  [2/10] 配置存在... "
if [[ -f "$cfg" ]]; then echo -e "${GREEN}${OK}${NC}"
else
echo -e "${RED}${FAIL} 不存在${NC}"; issues=$(( issues + 1 ))
confirm "  → 创建?" && { _create_minimal_config; fixed=$(( fixed + 1 )); }
fi
echo -ne "  [3/10] JSON有效... "
if [[ ! -f "$cfg" ]]; then echo -e "${DIM}跳过${NC}"
elif json_is_valid "$cfg"; then echo -e "${GREEN}${OK}${NC}"
else
echo -e "${RED}${FAIL} 损坏${NC}"; issues=$(( issues + 1 ))
confirm "  → 修复?" && { backup_config; _create_minimal_config; fixed=$(( fixed + 1 )); }
fi
echo -ne "  [4/10] gateway.bind... "
if [[ -f "$cfg" ]] && json_is_valid "$cfg" && has_cmd python3; then
local bv=""
...
echo -e "${RED}${FAIL} bind='$bv'无效${NC}"; issues=$(( issues + 1 ))
confirm "  → 修复?" && { sanitize_config_for_schema | sed 's/^/    /'; fixed=$(( fixed + 1 )); }
fi
else echo -e "${DIM}跳过${NC}"; fi
echo -ne "  [5/10] Schema验证... "
...
echo -e "${RED}${FAIL}${NC}"; issues=$(( issues + 1 ))
confirm "  → 自动修复?" && { openclaw_cmd doctor --fix >> "$LOG_FILE" 2>&1 || true; sanitize_config_for_schema | sed 's/^/    /'; fixed=$(( fixed + 1 )); }
else echo -e "${GREEN}${OK}${NC}"; fi
else echo -e "${DIM}跳过${NC}"; fi
echo -ne "  [6/10] DockerURL兼容... "
...
echo -e "${RED}${FAIL} LAN URL:$lu${NC}"; issues=$(( issues + 1 ))
confirm "  → 转换?" && { convert_urls_for_docker_mode | sed 's/^/    /'; fixed=$(( fixed + 1 )); }
fi
else echo -e "${DIM}跳过${NC}"; fi
echo -ne "  [7/10] Agent认证... "
if is_openclaw_installed; then
local ab=""
...
echo -e "${GREEN}${OK}${NC}"
else echo -e "${YELLOW}${WARN} 缺失${NC}"; issues=$(( issues + 1 )); fi
else echo -e "${DIM}跳过${NC}"; fi
echo -ne "  [8/10] Gateway端口(IPv4)... "
if gateway_health_check; then echo -e "${GREEN}${OK}${NC}"
else
echo -e "${YELLOW}${WARN} 无响应${NC}"; issues=$(( issues + 1 ))
confirm "  → 启动?" && { service_start; wait_gateway_ready 15 && fixed=$(( fixed + 1 )) || true; }
fi
echo -ne "  [9/10] IPv6支持... "
local ipv6=""
ipv6=$(get_local_ipv6)
if [[ -n "$ipv6" ]]; then
local v6_listen=false
has_cmd ss && ss -lntp 2>/dev/null | grep ":${OPENCLAW_PORT}" | grep -q ':::' && v6_listen=true || true
if $v6_listen; then
echo -e "${GREEN}${OK} ${ipv6}${NC}"
else
echo -e "${YELLOW}${WARN} 有IPv6地址但未监听(${ipv6})${NC}"
fi
else
echo -e "${DIM}跳过(无全局IPv6)${NC}"
fi
echo -ne "  [10/10] 磁盘空间... "
local avail=""
avail=$(df -k "$HOME" 2>/dev/null | awk 'NR==2{gsub(/[^0-9]/,"",$4); print ($4+0)}' || echo "0")
if [[ ! "$avail" =~ ^[0-9]+$ ]]; then
echo -e "${YELLOW}${WARN} 无法检测${NC}"
elif (( avail > 1048576 )); then
echo -e "${GREEN}${OK} $(df -h "$HOME" 2>/dev/null | awk 'NR==2{print $4}')${NC}"
else
echo -e "${RED}${FAIL} 空间不足(<1GB)${NC}"; issues=$(( issues + 1 ))
fi
if is_openclaw_installed; then
...
4) _container_exists "$DOCKER_CONTAINER" 2>/dev/null && _follow "docker logs -f '$DOCKER_CONTAINER' 2>&1" || msg_warn "官方版容器不存在" ;;
5) _container_exists "$DOCKER_CONTAINER_ZH" 2>/dev/null && _follow "docker logs -f '$DOCKER_CONTAINER_ZH' 2>&1" || msg_warn "中文版容器不存在" ;;
...
echo -e "\n${CYAN}IPv6相关${NC}"; print_line
_row "ip -6 addr show scope global" "查看IPv6地址"
_row "ss -lntp | grep ${OPENCLAW_PORT}" "查看端口监听(含IPv6)"
_row "curl http://[::1]:${OPENCLAW_PORT}/health" "测试IPv6连通性"
echo ""
wait_and_return 3
}
quick_commands() {
...
confirm "删除工作目录($DOCKER_DATA_DIR)?" && {
...
has_cmd openclaw \
&& msg_warn "openclaw 命令仍存在,可能需手动删除: $(command -v openclaw)" \
...
confirm "删除配置目录($OPENCLAW_CONFIG_DIR)? 包含所有配置和日志" && {
...
BANNER
echo -e "${NC}"
echo -e "  ${DIM}${SCRIPT_VERSION} | npm + Docker官方 + Docker中文版 | 多平台多架构 | IPv4/IPv6双栈${NC}\n"
...
local ipv6=""
ipv6=$(get_local_ipv6)
[[ -n "$ipv6" ]] && ipv6_label="  ${CYAN}IPv6✓${NC}"
echo -e "  GW:${sc}${st}${NC}  模式:${ml_c}${ml_txt}${NC}  配置:${cs_c}${cs_txt}${NC}  网络:${rg_c}${rg_txt}${NC}${ipv6_label}$(
